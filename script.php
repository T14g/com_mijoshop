...
class com_MijoshopInstallerScript {
    ...
    public function preflight($type, $parent) {
        ...
        if (!empty($config)) {
            ...
            if (version_compare($this->_current_version, '3.0.0', '<')){
                Jerror::raiseWarning(null, 'Direct upgrade from MijoShop 2.x to 3 is not possible. Please, check <a href="http://miwisoft.com/how-to-upgrade-from-mijoshop-2x-to-3x">this article</a> for further details ');
                return false;
            }
        }
    }
    ...
    protected function _installMijoshop() {
        ...
        ***********************************************************************************************/
        ...
        MijoShop::get('install')->createApiUser();
    ...
    protected function _updateMijoshop() {
        $this->creteUserOnUpdate();

        if (empty($this->_current_version)) {
            return;
        }

        if (version_compare($this->_current_version, '3.0.2') == -1) {
            MijoShop::get('install')->upgrade302();
        }

        if (version_compare($this->_current_version, '3.1.0') == -1) {
            MijoShop::get('install')->upgrade310();
        }

        if (version_compare($this->_current_version, '3.1.2') == -1) {
            MijoShop::get('install')->upgrade312();
        }

        if (version_compare($this->_current_version, '3.1.4') == -1) {
            MijoShop::get('install')->upgrade314();
        }

        if (version_compare($this->_current_version, '3.1.5') == -1) {
            MijoShop::get('install')->upgrade315();
        }

        if (version_compare($this->_current_version, '3.2.0') == -1) {
            MijoShop::get('install')->upgrade320();
        }
    }

    public function creteUserOnUpdate() {
        $db = JFactory::getDbo();
        $logged_user = JFactory::getUser();

        $db->setQuery("SELECT * FROM #__mijoshop_user AS u WHERE u.email = '".$logged_user->email."'");
        $user = $db->loadResult();
        ...
        $name       = explode(' ', $logged_user->name);
        $firstname  = $name[0];
        $lastname   = MijoShop::get('user')->getLastName($name);

        $db->setQuery("INSERT IGNORE INTO #__mijoshop_user SET ".
