...
class MijoShopUser {
    ...
    public function createOAccountFromJ($user, $is_login = false) {
		...
		if(JRequest::getString('option') == 'com_comprofiler') {
            $cache = array();
        }
        ...
        $phone = '';
        if (!empty($user['profile']) and is_array($user['profile']) and !empty($user['profile']['phone'])) {
            $phone = $db->run($user['profile']['phone'], 'escape');
        }
        ...
        if (!empty($user['password'])) {
            ...
            $password = $db->run($this->getEncryptedOPassword($user['email'], $user['password_clear']), 'escape');
        }
        ...
        if (!empty($customer_exists) and $is_login == true) {
            $db->run("UPDATE #__mijoshop_customer SET password = '{$password}' WHERE customer_id = " . $customer_id, 'query');

            $user_id = $this->getOUserIdFromJUser($user['id'], $username, $email);
            $user_exists = $this->getOUserById($user_id);

            if (!empty($user_exists)) {
                $db->run("UPDATE #__mijoshop_user SET password = '{$password}' WHERE user_id = " . $user_id, 'query');
            }
        ...
        if (!empty($customer_exists)) {
            ...
            $db->run("UPDATE #__mijoshop_customer SET firstname = '".$fname."', lastname = '".$lname."', email = '".$email."'".$phone.",  password = '".$password."', customer_group_id = '".$customer_group_id."', status = ".$status.", approved = ".$status." WHERE customer_id = '".$customer_id."'", 'query');
            ...
            $db->run("INSERT INTO #__mijoshop_customer SET firstname = '".$fname."', lastname = '".$lname."', email = '".$email."', telephone = '".$phone."', fax = '', password = '".$password."', newsletter = '0', customer_group_id = '".$customer_group_id."', status = '".$status."', approved = '".$status."', date_added = NOW()", 'query');
    ...
    public function loginOFromJ($_class, $user = null, $is_backend = false) {
        ...
        if (empty($user)) {
            ...
            $user['groups'] = $j_user->get('groups'); #mijoshop
	...
	public function createJUserFromO($userdata, $o_id) {
        $odb = MijoShop::get('db');
        $userdata = $this->cleanPostUserData($userdata);
        ...
        if(isset($userdata['customer_group_id'])) {
            $customer_group_info = MijoShop::get('opencart')->loadModelFunction('account/customer_group/getCustomerGroup', $userdata['customer_group_id'], false, 'catalog');
            $_activation = $customer_group_info['approval'];
        }
        else {
            $_activation = $params->get('useractivation', 1);
        }
    ...
    public function encryptPassword($password) {
        ...
        $is32  = MijoShop::get('base')->is32();
        $is2518 = MijoShop::get('base')->is2518();
        $isnot32 = MijoShop::get('base')->isnot32();

        if($isnot32 and $is2518){
            $phpass = new PasswordHash(10, true);
            $encrypted_password = $phpass->HashPassword($password);
        }
        else{
            $salt = JUserHelper::genRandomPassword(32);

           if ($joomlaPluginEnabled and $is32) {
               ...
               $defaultEncryption = $userPlugin->setDefaultEncryption($userPluginParams);
           }
           elseif ($is32) {
               $defaultEncryption = 'bcrypt';
           }
           else {
               $defaultEncryption = 'md5-hex';
           }

           $encrypted_password = JUserHelper::getCryptedPassword($password, $salt, $defaultEncryption);

           if($defaultEncryption ==  'md5-hex' and !strpos($encrypted_password, $salt)) {
               $encrypted_password = $encrypted_password. ':' .$salt;
           }
        }
	...
	public function syncPassword(&$user, $type = 'o2j')
    {
        $db = MijoShop::get('db');
        if ($type == 'o2j') {
            $oc_password = $db->run("SELECT password FROM #__mijoshop_customer WHERE email = '".$user->email."' LIMIT 1", 'loadResult');
            $user->password = $oc_password;
        }

        if(empty($user->password)){
            $oc_password = $db->run("SELECT password FROM #__mijoshop_user WHERE email = '".$user->email."' LIMIT 1", 'loadResult');
            $user->password = $oc_password;
        }
    }
    ...
    public function getEncryptedOPassword($var, $password, $func_suffix = 'Email') {
        ...
        if ($j_user) {
            ...
            if (substr($j_user['password'], 0, 3) == '$P$')
            {
                // Use PHPass's portable hashes with a cost of 10.
                $phpass = new PasswordHash(10, true);

                $match = $phpass->CheckPassword($password, $j_user['password']);

                if ($match) {
                    return $j_user['password'];
                }
                else {
                    return $password;
                }
            }
            else if (substr($j_user['password'], 0, 4) == '$2y$') {
                // BCrypt passwords are always 60 characters, but it is possible that salt is appended although non standard.
                $password60 = substr($j_user['password'], 0, 60);
            ...
            elseif (substr($j_user['password'], 0, 8) == '{SHA256}') {
                // Check the password
                $parts	= explode(':', $j_user['password']);
                $crypt	= $parts[0];
                $salt	= @$parts[1];
                $new_password = JUserHelper::getCryptedPassword($password, $salt, 'sha256', false);
                ...
                $new_password = JUserHelper::getCryptedPassword($password, $salt, 'md5-hex', false);
    ...
    private function cleanPostUserData($userdata) {
        $params = array('isRoot', 'groups', 'requireReset', '_params', '_authGroups', '_authLevels', '_authActions', '_errorMsg', '_errors', 'aid', 'otpKey', 'otep', 'resetCount', 'requireReset');

        foreach($params as $param) {
            if(isset($userdata[$param])){
                unset($userdata[$param]);
            }
        }
