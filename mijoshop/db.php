...
class MijoShopDb {
	...
	public function getRecordAlias($id, $type = 'product', $_query = array()) {
        ...
        if ( !isset($rows[$type][$id]) or !empty($_query['_lang'])) {
            $query = $type.'_id='.$id;

            $lang_id = MijoShop::get('opencart')->get('config')->get('config_language_id');

            if(isset($_query['_lang'])){
                $lang_id = $_query['_lang'];
            }

            $_name = $this->run("SELECT keyword FROM #__mijoshop_url_alias WHERE query = '{$query}' AND language_id = '{$lang_id}' LIMIT 1", 'loadResult');
    ...
    public function _getLanguageList($published = null) {
        ...
        foreach ($results as $result) {
            $directory = $this->_getOcDirectory($result['sef']);
            ...
            $_language_data[$result['sef']] = array(
                ...
                'directory'   => isset($directory[$result['sef']], $directory[$result['sef']]['directory']) ? $directory[$result['sef']]['directory'] : 'english',
                'filename'    => isset($directory[$result['sef']], $directory[$result['sef']]['directory']) ? $directory[$result['sef']]['directory'] : 'english',
    ...
    public function getOcLanguages() {
        ...
        foreach ($results as $result) {
            $language_data[$result['code']] = array(
                ...
                'image'       => str_replace('.png', '.gif', $result['image']),
    ...
    public function _getOcDirectory($code) {
        $language_data = array();

        $results = MijoShop::get('db')->run("SELECT * FROM #__mijoshop_language WHERE status = 1 AND code = '" . $code . "' ORDER BY sort_order, name", 'loadAssocList');
    ...
    public function getRecordAliases($id, $type = 'product') {
        ...
        if (!isset($rows[$type][$id])) {
            ...
            $_names = $this->run("SELECT keyword, language_id FROM #__mijoshop_url_alias WHERE query = '{$query}'", 'loadAssocList');
			$aliases = array();
