...
class MijoShopRouter extends MiwisoftComponentRouterBase
{
    ...
    public function buildRoute(&$query)
    {
        ...
        $requests = JFactory::getApplication()->input->getArray();

        if (isset($query['lang']) && $requests['language'] != $query['lang']) {
            foreach ($requests as $request_key => $request_value) {
                if ($request_key == 'language' || $request_key == 'mijoshop_store_id') {
                    continue;
                }

                $query[$request_key] = $request_value;
            }
        }
            ...
            switch ($query['route']) {
                case 'product/product':
                    ...
                    if (isset($query['product_id'])) {
                        $id = $query['product_id'];
                        $name = MijoShop::get('db')->getRecordAlias($id, 'product', $query);
                ...
                case 'product/category':
                    ...
                    if (isset($query['path'])) {
                        ...
                        $name = MijoShop::get('db')->getRecordAlias($id, 'category', $query);
                ...
                case 'product/manufacturer/info':
                    ...
                    if (isset($query['manufacturer_id'])) {
                        ...
                        $name = MijoShop::get('db')->getRecordAlias($id, 'manufacturer', $query);
                ...
                case 'information/information':
                    ...
                    if (isset($query['information_id'])) {
                        ...
                        $name = MijoShop::get('db')->getRecordAlias($id, 'information', $query);
    ...
    public function route($url)
    {
        ...
        if ($app->isSite()) {
            ...
            if (substr($url, 0, 10) == 'index.php?') {
                ...
                if (!isset($vars['Itemid'])) {
                    ...
                    if (MijoShop::get('base')->isAdmin('mijoshop')) {
                        ...
                        if ($vars['route'] == 'product/category') {
                            ...
                            if(JRequest::getVar('route', '') == 'product/category' and !empty($__itemid)) {
                                $Itemid = $__itemid;
                            }
                ...
                if (MijoShop::get('base')->is30() and isset($vars['route']) and !isset($_GET['product_id']) and isset($_GET['route']) and $vars['route'] == $_GET['route']) {
                    JRequest::set($vars, 'get');
                }
            }
