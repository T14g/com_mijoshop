...
class MijoShopUtility {
    ...
    public function getRemoteVersion() {
        ...
        $components = $this->getRemoteData('http://miwisoft.com/index.php?option=com_mijoextensions&view=xml&format=xml&catid=1');
        ...
        foreach ($category->children() as $component) {
            ...
            if (($option == 'com_mijoshop') and ($compability == 'all' or $compability == '3.0')) {
    ...
    public function getPackageFromUpload($userfile) {
        ...
        if (!$uploaded) {
            JError::raiseWarning(100, '<br /><br />' . JText::_('File not uploaded, please, make sure that your "MijoShop => Options => Personal ID" and/or the "Global Configuration => Server => Path to Temp-folder" field has a valid value.') . '<br /><br /><br />');
            return false;
        }
    ...
    public function getPackageFromServer($url) {
		...
		if (strpos($data, 'Error') !== false) {
		    JError::raiseWarning(100, JText::_('Please, enter your Personal-ID in MijoShop => Configuration page or upgrade manually (see the next tab).'));
		    return false;
	    }
        ...
        if (!$p_file) {
            JError::raiseWarning(100, JText::_('Invalid Personal ID'));
            return false;
        }
        ...
        if (!$package) {
            JError::raiseWarning(100, JText::_('An error occured, please, make sure that your "MijoShop => Options => Personal ID" and/or the "Global Configuration=>Server=>Path to Temp-folder" field has a valid value.'));
            return false;
        }
    ...
    public function synchronizeAliasManually(){
        $db         = MijoShop::get('db');
        $def_lang   = MijoShop::get('opencart')->get('config')->get('config_language_id');
        $languages  = $db->getLanguageList('all');

        if(count($languages) == 1){
            MijoShop::get('base')->setConfig('alias_sync_done', '1');
            return;
        }
		
		$check_column = $db->run("SELECT * FROM #__mijoshop_url_alias LIMIT 1", 'loadAssoc');
        
        if(empty($check_column) or (!empty($check_column) and !isset($check_column['language_id']))){
            $db->run("ALTER TABLE  #__mijoshop_url_alias ADD `language_id` INT NOT NULL DEFAULT 1;", 'query');
        }
		
        $aliases = $db->run("SELECT COUNT(url_alias_id) FROM `#__mijoshop_url_alias`", 'loadResult');

        if (!empty($aliases)) {
            foreach ($languages as $language) {
                if ($language['language_id'] == $def_lang) {
                    continue;
                }

                $is_sync = $db->run("SELECT COUNT(url_alias_id) FROM `#__mijoshop_url_alias` WHERE language_id = {$language['language_id']}", 'loadResult');

                if($is_sync > 0){
                    continue;
                }

                $db->run("INSERT INTO #__mijoshop_url_alias (query, keyword, language_id)
							SELECT query, keyword, ". $language['language_id'] ." as language_id
							FROM #__mijoshop_url_alias WHERE language_id = {$def_lang} AND query NOT LIKE 'manufacturer_id%'", 'query');
        ...
        $mainframe->redirect('index.php?option=com_mijoshop&ctrl=syncdone', JText::_('COM_MIJOSHOP_ALIAS_SYNC_DONE'));
    }
