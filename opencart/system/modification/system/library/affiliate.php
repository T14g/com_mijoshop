...
class Affiliate {
	...
	public function login($email, $password) {
        ...
        #mijoshop-end
		        ...
		        $affiliate_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "affiliate WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($password) . "'))))) OR password = '" . $this->db->escape(md5($password)) . "' OR password = '" . $encrypted_password . "' OR password = '" . $encrypted_password2 . "') AND status = '1' AND approved = '1'"); #mijoshop-start
