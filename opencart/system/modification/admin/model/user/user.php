...
class ModelUserUser extends Model {
	...
	public function editUser($user_id, $data) {
        ...
        MijoShop::get('user')->updateJUserFromO(MijoShop::get('user')->getJUserIdFromOUser($user_id, $data['username'], $data['email']), $data, $user_id); #mijoshop-start
		if ($data['password']) {
			$this->db->query("UPDATE `" . DB_PREFIX . "user` SET password = '" . $this->db->escape(Mijoshop::get('user')->encryptPassword($data['password'])) . "' WHERE user_id = '" . (int)$user_id . "'"); #mijoshop-start
		}
	}

	public function editPassword($user_id, $password) {
		$this->db->query("UPDATE `" . DB_PREFIX . "user` SET password = '" . $this->db->escape(Mijoshop::get('user')->encryptPassword($password)) . "', code = '' WHERE user_id = '" . (int)$user_id . "'"); #mijoshop-start
	}
	...
	public function deleteUser($user_id) {
        MijoShop::get('user')->deleteJUserFromO($user_id, true); #mijoshop-start
