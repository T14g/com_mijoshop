<?php echo $header; ?>

<?php echo $column_left; ?>
<div id="content">
<div class="page-header">
<div class="container-fluid">
<div class="pull-right">

            <button type="submit" onclick="save('save')" form="form-account" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="Save"><i class="fa fa-save"></i></button>
            <button type="submit" form="form-account" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="Save & Close"><i class="fa fa-sign-out"></i></button>
            
<a href="<?php echo $cancel; ?>" data-toggle="tooltip" title="<?php echo $button_cancel; ?>" class="btn btn-default"><i class="fa fa-reply"></i></a></div>
<h1><?php echo $heading_title; ?></h1>
<ul class="breadcrumb">
<?php foreach ($breadcrumbs as $breadcrumb) { ?>
<li><a href="<?php echo $breadcrumb['href']; ?>"><?php echo $breadcrumb['text']; ?></a></li>
<?php } ?>
</ul>
</div>
</div>
<div class="container-fluid">
<?php if ($error_warning) { ?>
<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> <?php echo $error_warning; ?>
<button type="button" class="close" data-dismiss="alert">&times;</button>
</div>
<?php } ?>
<div class="panel panel-default">
<div class="panel-heading">
<h3 class="panel-title"><i class="fa fa-pencil"></i> Configura&ccedil;&otilde;es</h3>
</div>
<div class="panel-body">

<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-account" class="form-horizontal">

<div role="tabpanel">
<!-- Nav tabs -->
<ul class="nav nav-tabs" role="tablist">
<li role="presentation" class="active"><a href="#configuracoes" aria-controls="configuracoes" role="tab" data-toggle="tab">Configura&ccedil;&otilde;es</a></li>
<li role="presentation"><a href="#campos" aria-controls="sobre" role="tab" data-toggle="tab">Campos Customizados</a></li>
<li role="presentation"><a href="#facebook" aria-controls="sobre" role="tab" data-toggle="tab">Facebook Login</a></li>
<li role="presentation"><a href="#checkout" aria-controls="sobre" role="tab" data-toggle="tab">Checkout</a></li>
<li role="presentation"><a href="#custom" aria-controls="sobre" role="tab" data-toggle="tab">Custom CSS e JS</a></li>
<li role="presentation"><a href="#pagamento" aria-controls="pagamento" role="tab" data-toggle="tab">Meios Pagamento</a></li>
<li role="presentation"><a href="#sobre" aria-controls="sobre" role="tab" data-toggle="tab">Sobre</a></li>
</ul>
<div class="tab-content">

<div role="tabpanel" class="tab-pane active" id="configuracoes">
<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Serial</label>
<div class="col-sm-9">
<input type="text" class="form-control" name="compraexpress_serial" value="<?php echo $compraexpress_serial;?>">
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status"><?php echo $entry_status; ?></label>
<div class="col-sm-9">
<select name="compraexpress_status" id="input-status" class="form-control">
<?php if ($compraexpress_status) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Exibir Erros Form</label>
<div class="col-sm-9">
<select name="compraexpress_erro" id="input-status" class="form-control">
<?php if ($compraexpress_erro) { ?>
<option value="1" selected="selected">Inline (direto no form)</option>
<option value="0">Popup Modal</option>
<?php } else { ?>
<option value="1">Inline (direto no form)</option>
<option value="0" selected="selected">Popup Modal</option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Modelo Checkout</label>
<div class="col-sm-9">
<select name="compraexpress_modelo" id="input-status" class="form-control">
<?php if ($compraexpress_modelo) { ?>
<option value="1" selected="selected">Modelo 01</option>
<option value="0">Modelo 02</option>
<?php } else { ?>
<option value="1">Modelo 01</option>
<option value="0" selected="selected">Modelo 02</option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Grupo Pessoa Fisica</label>
<div class="col-sm-9">
<select name="compraexpress_grupopf" id="input-status" class="form-control">
<?php foreach($grupos AS $k=>$v){?>
<?php if ($compraexpress_grupopf==$v['customer_group_id']) { ?>
<option value="<?php echo $v['customer_group_id'];?>" selected="selected"><?php echo $v['name'];?></option>
<?php } else { ?>
<option value="<?php echo $v['customer_group_id'];?>"><?php echo $v['name'];?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Grupo Pessoa Juridica</label>
<div class="col-sm-9">
<select name="compraexpress_grupopj" id="input-status" class="form-control">
<?php foreach($grupos AS $k=>$v){?>
<?php if ($compraexpress_grupopj==$v['customer_group_id']) { ?>
<option value="<?php echo $v['customer_group_id'];?>" selected="selected"><?php echo $v['name'];?></option>
<?php } else { ?>
<option value="<?php echo $v['customer_group_id'];?>"><?php echo $v['name'];?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>


</div>

<div role="tabpanel" class="tab-pane" id="checkout">
<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">&nbsp;</label>
<div class="col-sm-9">
<div class="alert alert-info" role="alert">Op&ccedil;&otilde;es Checkout Express.</div>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Checkout sem Login</label>
<div class="col-sm-9">
<select name="compraexpress_log" id="input-status" class="form-control">
<?php if ($compraexpress_log) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Permitir Comentario</label>
<div class="col-sm-9">
<select name="compraexpress_com" id="input-status" class="form-control">
<?php if ($compraexpress_com) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Redirecionar Checkout ao Comprar</label>
<div class="col-sm-9">
<select name="compraexpress_redirecionar" id="input-status" class="form-control">
<?php if ($compraexpress_redirecionar) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">&nbsp;</label>
<div class="col-sm-9">
<div class="alert alert-info" role="alert">CSS, Topo e Rodap&eacute; Checkout Express.</div>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Topo e rodap&eacute;</label>
<div class="col-sm-9">
<select name="compraexpress_te" id="input-status" class="form-control">
<?php if ($compraexpress_te) { ?>
<option value="1" selected="selected">Padr&atilde;o loja</option>
<option value="0">Padr&atilde;o checkout</option>
<?php } else { ?>
<option value="1">Padr&atilde;o loja</option>
<option value="0" selected="selected">Padr&atilde;o checkout</option>
<?php } ?>
</select>
</div>
</div>

<?php if(isset($temas['themes'])){?>
<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Custom Layout (padr&atilde;o checkout)</label>
<div class="col-sm-9">
<select name="compraexpress_tema" id="input-status" class="form-control">
<option value="">Padr&atilde;o</option>
<?php foreach($temas['themes'] AS $k=>$v) { ?>
<option value="<?php echo $v['cssCdn'];?>"<?php echo ($v['cssCdn']==$compraexpress_tema)?' selected':'';?>><?php echo $v['name']; ?></option>
<?php } ?>
</select>
<span id="helpBlock" class="help-block">Somente se usar "Padr&atilde;o checkout".</span>
</div>
</div>
<?php } ?>
</div>

<div role="tabpanel" class="tab-pane" id="facebook">
<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">&nbsp;</label>
<div class="col-sm-9">
<div class="alert alert-info" role="alert">Para criar um APP de login Facebook <a href="https://developers.facebook.com/quickstarts/?platform=web" target="_blank">clique aqui</a> e crie/edite um para LOGIN no website.</div>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Login Facebook</label>
<div class="col-sm-9">
<select name="compraexpress_fb" id="input-status" class="form-control">
<?php if ($compraexpress_fb) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">APP ID</label>
<div class="col-sm-9">
<input type="text" class="form-control" name="compraexpress_fbid" value="<?php echo $compraexpress_fbid;?>">
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">APP Secret</label>
<div class="col-sm-9">
<input type="text" class="form-control" name="compraexpress_fbsec" value="<?php echo $compraexpress_fbsec;?>">
</div>
</div>
</div>

<div role="tabpanel" class="tab-pane" id="custom">
<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Custom CSS</label>
<div class="col-sm-9">
<textarea class="form-control" style="height:180px;" name="compraexpress_css"><?php echo $compraexpress_css;?></textarea>
<span id="helpBlock" class="help-block">N&atilde;o requer informar as tags &lt;styles&gt;&lt;/styles&gt;.</span>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-description">Custom Javascript</label>
<div class="col-sm-9">
<textarea name="compraexpress_js" style="height:180px;" id="selos" class="form-control"><?php echo $compraexpress_js; ?>
</textarea>
<span id="helpBlock" class="help-block">N&atilde;o requer informar as tags &lt;script&gt;&lt;/script&gt;.</span>
</div>
</div>


<?php
$selo_padrao = '<div style="text-align: left;"><span style="font-weight: bold;">COMPRE COM SEGURANÇA</span></div>
<div style="text-align: left;"><img src="'.HTTPS_CATALOG.'image/catalog/site-seguro.png" style="width: 184.5px; height: 87.573732718894px;"></div>';
$pag_padrao = '<p><span style="font-weight: bold;">FORMAS DE PAGAMENTO<br><img src="'.HTTPS_CATALOG.'image/catalog/formas-de-pagamento.png"><br><br>FORMAS DE ENTREGA<br><img src="'.HTTPS_CATALOG.'image/catalog/forma-envio.png" style="width: 254.5px; height: 29.5594900849858px;"><br></span></p>';
?>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-description">HTML 1</label>
<div class="col-sm-9">
<textarea name="compraexpress_selos" id="selos" class="html_ok form-control"><?php echo empty($compraexpress_selos)?$selo_padrao:$compraexpress_selos; ?>
</textarea>
<span id="helpBlock" class="help-block">Somente se usar "Padr&atilde;o checkout".</span>
</div>
</div>


<div class="form-group">
<label class="col-sm-3 control-label" for="input-description">HTML 2</label>
<div class="col-sm-9">
<textarea name="compraexpress_pag" id="selos" class="html_ok form-control"><?php echo empty($compraexpress_pag)?$pag_padrao:$compraexpress_pag; ?>
</textarea>
<span id="helpBlock" class="help-block">Somente se usar "Padr&atilde;o checkout".</span>
</div>
</div>
</div>

<div role="tabpanel" class="tab-pane" id="campos">
<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">&nbsp;</label>
<div class="col-sm-9">
<div class="alert alert-info" role="alert">Cadastre os campos customizados em "Clientes" > "Campos customizados".</div>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Tipo de cadastro <font color="red">*</font></label>
<div class="col-sm-9">
<select name="compraexpress_tipo" id="input-status" class="form-control">

<?php 
$tipos[] = array('id'=>0,'name'=>'Pessoa fisica / Pessoa juridica');
$tipos[] = array('id'=>1,'name'=>'Somente pessoa fisica');
$tipos[] = array('id'=>2,'name'=>'Somente pessoa juridica');
foreach($tipos AS $k=>$v){
?>
<?php if ($compraexpress_tipo==$v['id']) { ?>
<option value="<?php echo $v['id'];?>" selected="selected"><?php echo $v['name'];?></option>
<?php } else { ?>
<option value="<?php echo $v['id'];?>"><?php echo $v['name'];?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Campo Conta > CPF/CNPJ <font color="red">*</font></label>
<div class="col-sm-9">
<select name="compraexpress_fiscal" id="input-status" class="form-control">
<option value="">Selecione</option>
<?php foreach($campos AS $k=>$v){?>
<?php if ($compraexpress_fiscal==$v['custom_field_id']) { ?>
<option value="<?php echo $v['custom_field_id'];?>" selected="selected"><?php echo $v['name'];?></option>
<?php } else { ?>
<option value="<?php echo $v['custom_field_id'];?>"><?php echo $v['name'];?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Campo Endere&ccedil;o > N&uacute;mero <font color="red">*</font></label>
<div class="col-sm-9">
<select name="compraexpress_numero" id="input-status" class="form-control">
<option value="">Selecione</option>
<?php foreach($campos AS $k=>$v){?>
<?php if ($compraexpress_numero==$v['custom_field_id']) { ?>
<option value="<?php echo $v['custom_field_id'];?>" selected="selected"><?php echo $v['name'];?></option>
<?php } else { ?>
<option value="<?php echo $v['custom_field_id'];?>"><?php echo $v['name'];?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-3 control-label" for="input-status">Campo Endere&ccedil;o > Complemento <font color="red">*</font></label>
<div class="col-sm-9">
<select name="compraexpress_complemento" id="input-status" class="form-control">
<option value="">Selecione</option>
<?php foreach($campos AS $k=>$v){?>
<?php if ($compraexpress_complemento==$v['custom_field_id']) { ?>
<option value="<?php echo $v['custom_field_id'];?>" selected="selected"><?php echo $v['name'];?></option>
<?php } else { ?>
<option value="<?php echo $v['custom_field_id'];?>"><?php echo $v['name'];?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>
</div>

<div role="tabpanel" class="tab-pane" id="pagamento">
<div class="form-group">
<div class="col-sm-12">
<table class="table table-bordered">
<thead>
<tr>
<th width="250">Forma de Pagamento</th>
<th>Imagem a exibir (deixe vazio para usar o titulo em texto padr&atilde;o)</th>
</tr>
</thead>
	  
<?php 
foreach($meios AS $k=>$v){
?>
<tr>
<td><?php echo str_replace('_',' ',strtoupper($v));?></td>
<td><input type="text" name="compraexpress_pagamentos[<?php echo $v;?>]" value="<?php echo isset($compraexpress_pagamentos[$v])?$compraexpress_pagamentos[$v]:''; ?>" placeholder="http://www.sualoja.com/imagem.png" style="width:100%" class="form-control" /></td>
</tr>
<?php } ?>

</table>
</div>
</div>
</div>

<div role="tabpanel" class="tab-pane" id="sobre">
<img src="data:image/jpg;base64,"/>
<br><br>
<b>Vers&atilde;o:</b> 4.0<br>
<b>Data:</b> 19/09/2016<br>
<br>
Suporte e Vendas: <a href="http://www.loja5.com.br/" target="_blank">http://www.loja5.com.br/</a><br>
Contato: <u>suportedaloja@gmail.com</u><br>
</div>		


</div>
</div>

</form>

</div>
</div>
</div>
</div>
<script type="text/javascript">
<!-- 
$('.html_ok').summernote({
	height: 150,
	codemirror: { // codemirror options
		theme: 'monokai'
	}
});
//-->
</script>

        <script type="text/javascript"><!--
        function save(type){
            var input = document.createElement('input');
            input.type = 'hidden';
            input.name = 'button';
            input.value = type;
            form = $("form[id^='form-']").append(input);
            form.submit();
        }
        //--></script>
            
<?php echo $footer; ?>