<?php echo $header; ?>

<?php
function getStatusCielo5($status) {
	switch($status) {
		case "0": $status = "Aguardando Pagamento";
		break;
		case "1": $status = "Autorizado";
		break;
		case "2": $status = "Pagamento Confirmado";
		break;
		case "3": $status = "Negado";
		break;
		case "20": $status = "Agendado";
		break;
		case "13": $status = "Abortado";
		break;
		case "12": $status = "Pendente";
		break;
		case "11": $status = "Devolvido";
		break;
		case "10": $status = "Cancelado";
		break;
		default: $status = "N&atilde;o Finalizado";
		break;
	}
	return $status;
}
?>

<link href="https://cdn.datatables.net/1.10.10/css/dataTables.bootstrap.min.css" rel="stylesheet" />

<script type="text/javascript" src="//cdn.jsdelivr.net/jquery.maskmoney/3.0.2/jquery.maskMoney.min.js"></script>

<script type="text/javascript" src="https://cdn.datatables.net/1.10.10/js/jquery.dataTables.min.js"></script>

<script type="text/javascript" src="https://cdn.datatables.net/1.10.10/js/dataTables.bootstrap.min.js"></script>
<script>
$(function(){
$(".dinheiro").maskMoney({thousands:'', decimal:'.', allowZero:true, suffix: ''});
});
</script>

<?php echo $column_left; ?>

<div id="content">
<div class="page-header">
<div class="container-fluid">
<div class="pull-right">
<?php if(!isset($_GET['pedidos'])){ ?>

            <button type="submit" onclick="save('save')" form="form-cielo5" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="Save"><i class="fa fa-save"></i></button>
            <button type="submit" form="form-cielo5" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="Save & Close"><i class="fa fa-sign-out"></i></button>
            
<a href="<?php echo $cancel; ?>" data-toggle="tooltip" title="<?php echo $button_cancel; ?>" class="btn btn-default"><i class="fa fa-reply"></i></a><?php } ?></div>
<h1><?php echo $heading_title; ?></h1>
<ul class="breadcrumb">
<?php foreach ($breadcrumbs as $breadcrumb) { ?>
<li><a href="<?php echo $breadcrumb['href']; ?>"><?php echo $breadcrumb['text']; ?></a></li>
<?php } ?>
</ul>
</div>
</div>
<div class="container-fluid">
<?php if ($error_warning) { ?>
<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> <?php echo $error_warning; ?>
<button type="button" class="close" data-dismiss="alert">&times;</button>
</div>
<?php } ?>
<div class="panel panel-default">
<div class="panel-heading">
<h3 class="panel-title"><i class="fa fa-list"></i> Configura&ccedil;&otilde;es</h3>
</div>
<div class="panel-body">


<div role="tabpanel">

<ul class="nav nav-tabs" id="tabelas" role="tablist">
<?php if(!isset($_GET['pedidos'])){?>
<li role="presentation" class="active"><a href="#configuracoes" aria-controls="configuracoes" role="tab" data-toggle="tab">Configura&ccedil;&otilde;es</a></li>
<li role="presentation"><a href="#parcelas" aria-controls="parcelas" role="tab" data-toggle="tab">Cart&atilde;o de Cr&eacute;dito</a></li>
<li role="presentation"><a href="#debito" aria-controls="debito" role="tab" data-toggle="tab">Cart&atilde;o de D&eacute;bito</a></li>
<li role="presentation"><a href="#boleto" aria-controls="boleto" role="tab" data-toggle="tab">Boleto</a></li>
<li role="presentation"><a href="#tef" aria-controls="tef" role="tab" data-toggle="tab">TEF</a></li>
<li role="presentation"><a href="#status" aria-controls="status" role="tab" data-toggle="tab">Status</a></li>
<?php } ?>
<li role="presentation"><a href="#pedidos" aria-controls="status" role="tab" data-toggle="tab">Pedidos</a></li>
<li role="presentation"><a href="#sobre" aria-controls="sobre" role="tab" data-toggle="tab">Sobre</a></li>
</ul>

<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-cielo5" class="form-horizontal">

<div class="tab-content">

<?php if(!isset($_GET['pedidos'])){ ?>

<div role="tabpanel" class="tab-pane active" id="configuracoes">	  
<div class="form-group">
<label class="col-sm-2 control-label" for="input">Serial (<?php echo $mod_ativado;?>)</label>
<div class="col-sm-10">
<input type="text" name="braspag5_serial" value="<?php echo $braspag5_serial; ?>" placeholder="Vendas em www.loja5.com.br" id="input" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Status Geral</label>
<div class="col-sm-10">
<select name="braspag5_status" id="input-status" class="form-control">
<?php if ($braspag5_status) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-geo-zone">Zona</label>
<div class="col-sm-10">
<select name="braspag5_geo_zone_id" id="input-geo-zone" class="form-control">
<option value="0">Todas</option>
<?php foreach ($geo_zones as $geo_zone) { ?>
<?php if ($geo_zone['geo_zone_id'] == $braspag5_geo_zone_id) { ?>
<option value="<?php echo $geo_zone['geo_zone_id']; ?>" selected="selected"><?php echo $geo_zone['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $geo_zone['geo_zone_id']; ?>"><?php echo $geo_zone['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Modo</label>
<div class="col-sm-10">
<select name="braspag5_modo" id="input-status" class="form-control">
<?php if ($braspag5_modo) { ?>
<option value="1" selected="selected">Produ&ccedil;&atilde;o</option>
<option value="0">Teste</option>
<?php } else { ?>
<option value="1">Produ&ccedil;&atilde;o</option>
<option value="0" selected="selected">Teste</option>
<?php } ?>
</select>
</div>
</div>

<input type="hidden" name="braspag5_operadora" value="cielo">

<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret"><span data-toggle="tooltip" title="Numero de integracao/afiliacao fornecido pela cielo.">MerchantID</span></label>
<div class="col-sm-10">
<input type="text" name="braspag5_id" value="<?php echo $braspag5_id; ?>" placeholder="0000000000" id="input-secret" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret"><span data-toggle="tooltip" title="Fornecida pela cielo.">MerchantKey</span></label>
<div class="col-sm-10">
<input type="text" name="braspag5_senha" value="<?php echo $braspag5_senha; ?>" placeholder="**********************" maxlength="80" id="input-secret" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-canceled-status">Origem CPF/CNPJ</label>
<div class="col-sm-10">
<select name="braspag5_cpf" id="input-canceled-status" class="form-control">
<option value="0" selected="selected">Cliente digita manual</option>
<?php foreach ($campos as $campo) { ?>
<?php if ($campo['custom_field_id'] == $braspag5_cpf) { ?>
<option value="<?php echo $campo['custom_field_id']; ?>" selected="selected"><?php echo $campo['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $campo['custom_field_id']; ?>"><?php echo $campo['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-canceled-status">Origem N&uacute;mero</label>
<div class="col-sm-10">
<select name="braspag5_numero" id="input-canceled-status" class="form-control">
<option value="0" selected="selected">Vai junto ao logradouro</option>
<?php foreach ($campos as $campo) { ?>
<?php if ($campo['custom_field_id'] == $braspag5_numero) { ?>
<option value="<?php echo $campo['custom_field_id']; ?>" selected="selected"><?php echo $campo['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $campo['custom_field_id']; ?>"><?php echo $campo['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-canceled-status">Origem Complemento</label>
<div class="col-sm-10">
<select name="braspag5_complemento" id="input-canceled-status" class="form-control">
<option value="0" selected="selected">Nao requer o uso</option>
<?php foreach ($campos as $campo) { ?>
<?php if ($campo['custom_field_id'] == $braspag5_complemento) { ?>
<option value="<?php echo $campo['custom_field_id']; ?>" selected="selected"><?php echo $campo['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $campo['custom_field_id']; ?>"><?php echo $campo['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>


<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Ativar Debug</label>
<div class="col-sm-10">
<select name="braspag5_debug" id="input-status" class="form-control">
<?php if ($braspag5_debug) { ?>
<option value="1" selected="selected">Sim</option>
<option value="0">N&atilde;o</option>
<?php } else { ?>
<option value="1">Sim</option>
<option value="0" selected="selected">N&atilde;o</option>
<?php } ?>
</select>
</div>
</div>
</div>

<div role="tabpanel" class="tab-pane" id="debito">
<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Status</label>
<div class="col-sm-10">
<select name="braspag5_status_debito" id="input-status" class="form-control">
<?php if ($braspag5_status_debito) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input">Titulo</label>
<div class="col-sm-10">
<input type="text" name="braspag5_titulo_debito" value="<?php echo $braspag5_titulo_debito; ?>" placeholder="Nome do meio a mostra ao cliente" id="input" class="form-control" />
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-sort-order">Ordem</label>
<div class="col-sm-10">
<input type="text" name="braspag5_ordem_debito" value="<?php echo $braspag5_ordem_debito; ?>" placeholder="0" id="input-sort-order" class="form-control" />
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-total"><span data-toggle="tooltip" title="Valor minimo de um pedido para que o cliente possa finalizar usanto este meio de pagamento.">Total minimo</span></label>
<div class="col-sm-10">
<input type="text" name="braspag5_total_debito" value="<?php echo $braspag5_total_debito; ?>" placeholder="0.00" id="input-total" class="dinheiro form-control" />
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input">Bandeiras</label>
<div class="col-sm-10">
<?php
if(empty($braspag5_provide_debito)){
	$braspag5_provide_debito = array();
}
?>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_provide_debito[]" id="input" value="electron"<?php echo in_array('electron',$braspag5_provide_debito)?' checked':'';?>> Visa Electron
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_provide_debito[]" id="input" value="maestro"<?php echo in_array('maestro',$braspag5_provide_debito)?' checked':'';?>> Maestro<br>
</label>
</div>
</div>
</div>
</div>

<div role="tabpanel" class="tab-pane" id="boleto">
<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Status</label>
<div class="col-sm-10">
<select name="braspag5_status_boleto" id="input-status" class="form-control">
<?php if ($braspag5_status_boleto) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input">Titulo</label>
<div class="col-sm-10">
<input type="text" name="braspag5_titulo_boleto" value="<?php echo $braspag5_titulo_boleto; ?>" placeholder="Nome do meio a mostra ao cliente" id="input" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-sort-order">Ordem</label>
<div class="col-sm-10">
<input type="text" name="braspag5_ordem_boleto" value="<?php echo $braspag5_ordem_boleto; ?>" placeholder="0" id="input-sort-order" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-total"><span data-toggle="tooltip" title="Valor minimo de um pedido para que o cliente possa finalizar usanto este meio de pagamento.">Total minimo</span></label>
<div class="col-sm-10">
<input type="text" name="braspag5_total_boleto" value="<?php echo $braspag5_total_boleto; ?>" placeholder="0.00" id="input-total" class="dinheiro form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input">Banco</label>
<div class="col-sm-10">
<select name="braspag5_provide_boleto" id="input" class="form-control">
<?php 
$arraybancos = array('Bradesco','Bradesco2','BancoDoBrasil','BancoDoBrasil2');
foreach($arraybancos AS $k=>$v){
?>
<?php if ($v==$braspag5_provide_boleto) { ?>
<option value="<?php echo $v;?>" selected="selected"><?php echo $v;?></option>
<?php } else { ?>
<option value="<?php echo $v;?>"><?php echo $v;?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input">Cedente</label>
<div class="col-sm-10">
<input type="text" name="braspag5_boleto_cedente" value="<?php echo $braspag5_boleto_cedente; ?>" placeholder="Nome do cedente" id="input" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input">Instru&ccedil;&otilde;es</label>
<div class="col-sm-10">
<input type="text" name="braspag5_boleto_instrucoes" value="<?php echo $braspag5_boleto_instrucoes; ?>" placeholder="" id="input" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input">Demostrativo</label>
<div class="col-sm-10">
<input type="text" name="braspag5_boleto_demo" value="<?php echo $braspag5_boleto_demo; ?>" placeholder="" id="input" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input">Validade em Dias</label>
<div class="col-sm-10">
<input type="number" name="braspag5_boleto_dias" value="<?php echo $braspag5_boleto_dias; ?>" placeholder="5" id="input" class="form-control" />
</div>
</div>
</div>

<div role="tabpanel" class="tab-pane" id="tef">
<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Status</label>
<div class="col-sm-10">
<select name="braspag5_status_tef" id="input-status" class="form-control">
<?php if ($braspag5_status_tef) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input">Titulo</label>
<div class="col-sm-10">
<input type="text" name="braspag5_titulo_tef" value="<?php echo $braspag5_titulo_tef; ?>" placeholder="Nome do meio a mostra ao cliente" id="input" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-sort-order">Ordem</label>
<div class="col-sm-10">
<input type="text" name="braspag5_ordem_tef" value="<?php echo $braspag5_ordem_tef; ?>" placeholder="0" id="input-sort-order" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-total"><span data-toggle="tooltip" title="Valor minimo de um pedido para que o cliente possa finalizar usanto este meio de pagamento.">Total minimo</span></label>
<div class="col-sm-10">
<input type="text" name="braspag5_total_tef" value="<?php echo $braspag5_total_tef; ?>" placeholder="0.00" id="input-total" class="dinheiro form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input">Banco</label>
<div class="col-sm-10">
<?php
if(empty($braspag5_provide_tef)){
$braspag5_provide_tef = array();
}
$arraybancos = array('Bradesco'=>'Bradesco','BancoDoBrasil'=>'Banco do Brasil');
foreach($arraybancos AS $k=>$v){
?>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_provide_tef[]" value="<?php echo $k;?>"<?php echo in_array($k,$braspag5_provide_tef)?' checked':'';?>> <?php echo $v;?>
</label>
</div>
<?php } ?>
</div>
</div>

</div>

<div role="tabpanel" class="tab-pane" id="parcelas">
<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Status</label>
<div class="col-sm-10">
<select name="braspag5_status_cartao" id="input-status" class="form-control">
<?php if ($braspag5_status_cartao) { ?>
<option value="1" selected="selected"><?php echo $text_enabled; ?></option>
<option value="0"><?php echo $text_disabled; ?></option>
<?php } else { ?>
<option value="1"><?php echo $text_enabled; ?></option>
<option value="0" selected="selected"><?php echo $text_disabled; ?></option>
<?php } ?>
</select>
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input">Titulo</label>
<div class="col-sm-10">
<input type="text" name="braspag5_titulo_cartao" value="<?php echo $braspag5_titulo_cartao; ?>" placeholder="Nome do meio a mostra ao cliente" id="input" class="form-control" />
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-sort-order">Ordem</label>
<div class="col-sm-10">
<input type="text" name="braspag5_ordem_cartao" value="<?php echo $braspag5_ordem_cartao; ?>" placeholder="0" id="input-sort-order" class="form-control" />
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-total"><span data-toggle="tooltip" title="Valor minimo de um pedido para que o cliente possa finalizar usanto este meio de pagamento.">Total minimo</span></label>
<div class="col-sm-10">
<input type="text" name="braspag5_total_cartao" value="<?php echo $braspag5_total_cartao; ?>" placeholder="0.00" id="input-total" class="dinheiro form-control" />
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret"><span data-toggle="tooltip" title="O numero maximo de parcelar qual ira dividir em sua loja, lembre-se que deve ser maior ou igual a dividir sem juros e de acordo o seu contrato junto a operadora.">Dividir no maximo</span></label>
<div class="col-sm-10">
<input type="number" min="1" max="12" name="braspag5_div" value="<?php echo $braspag5_div; ?>" placeholder="De 1 a 12" maxlength="2" id="input-secret" class="form-control" />
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret"><span data-toggle="tooltip" title="O numero maximo de parcelar qual ira dividir sem cobrar juros em sua loja, lembre-se que deve ser menor ou igual a dividir e de acordo seu contrato com a operadora.">Dividir sem juros</span></label>
<div class="col-sm-10">
<input type="number" min="1" max="12" name="braspag5_sem" value="<?php echo $braspag5_sem; ?>" placeholder="De 1 a 12" maxlength="2" id="input-secret" class="form-control" />
</div>
</div>
<?php
if(empty($braspag5_meios)){
	$braspag5_meios = array();
}
?>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret">Bandeiras aceitas</label>
<div class="col-sm-10">

<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="visa"<?php echo in_array('visa',$braspag5_meios)?' checked':'';?>> Visa
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="mastercard"<?php echo in_array('mastercard',$braspag5_meios)?' checked':'';?>> Mastercard
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="elo"<?php echo in_array('elo',$braspag5_meios)?' checked':'';?>> Elo
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="diners"<?php echo in_array('diners',$braspag5_meios)?' checked':'';?>> Diners
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="discover"<?php echo in_array('discover',$braspag5_meios)?' checked':'';?>> Discover
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="amex"<?php echo in_array('amex',$braspag5_meios)?' checked':'';?>> Amex
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="jcb"<?php echo in_array('jcb',$braspag5_meios)?' checked':'';?>> JCB
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="aura"<?php echo in_array('aura',$braspag5_meios)?' checked':'';?>> Aura
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="hiper"<?php echo in_array('hiper',$braspag5_meios)?' checked':'';?>> Hiper
</label>
</div>
<div class="checkbox">
<label>
<input type="checkbox" name="braspag5_meios[]" value="hipercard"<?php echo in_array('hipercard',$braspag5_meios)?' checked':'';?>> Hipercard
</label>
</div>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret">Parcela minima</label>
<div class="col-sm-10">
<input type="text" name="braspag5_minimo" value="<?php echo $braspag5_minimo; ?>" placeholder="No minimo 5.00" maxlength="10" id="input-secret" class="dinheiro form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret">Taxa % de juros</label>
<div class="col-sm-10">
<input type="text" name="braspag5_juros" value="<?php echo $braspag5_juros; ?>" placeholder="0.00" maxlength="10" id="input-secret" class="dinheiro form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Parcelamento com Juros</label>
<div class="col-sm-10">
<select name="braspag5_tipo_parcelamento" id="input-status" class="form-control">
<?php if ($braspag5_tipo_parcelamento==3) { ?>
<option value="3" selected="selected">Operadora</option>
<option value="2">Loja</option>
<?php } else { ?>
<option value="3">Operadora</option>
<option value="2" selected="selected">Loja</option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Tipo de captura</label>
<div class="col-sm-10">
<select name="braspag5_tipo_captura" id="input-status" class="form-control">
<?php if ($braspag5_tipo_captura=='true') { ?>
<option value="true" selected="selected">Automatica</option>
<option value="false">Posterior</option>
<?php } else { ?>
<option value="true">Automatica</option>
<option value="false" selected="selected">Posterior</option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret"><span data-toggle="tooltip" title="Nome a exibir na fatura do cartao do cliente. Usar somente letras e numeros e de no maximo 13 digitos.">Soft Descripto</span></label>
<div class="col-sm-10">
<input type="text" name="braspag5_soft" value="<?php echo $braspag5_soft; ?>" placeholder="MINHALOJA" maxlength="13" id="input-secret" class="form-control" />
</div>
</div>

<input type="hidden" name="braspag5_provide_cartao" value="Cielo">

<div class="form-group">
<label class="col-sm-2 control-label" for="input-status">Ativar Anti-fraude</label>
<div class="col-sm-10">
<select name="braspag5_antifraude" id="input-status" class="form-control">
<?php if ($braspag5_antifraude) { ?>
<option value="1" selected="selected">Sim</option>
<option value="0">N&atilde;o</option>
<?php } else { ?>
<option value="1">Sim</option>
<option value="0" selected="selected">N&atilde;o</option>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret">Fraude MID (opcional)</label>
<div class="col-sm-10">
<input type="text" name="braspag5_mid" value="<?php echo $braspag5_mid; ?>" placeholder="" id="input-secret" class="form-control" />
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-secret">Fraude Org ID (opcional)</label>
<div class="col-sm-10">
<input type="text" name="braspag5_orgid" value="<?php echo $braspag5_orgid; ?>" placeholder="" id="input-secret" class="form-control" />
</div>
</div>

</div>

<div role="tabpanel" class="tab-pane" id="status">
<div class="form-group">
<label class="col-sm-2 control-label" for="input-order-status">Aguardando Pagamento</label>
<div class="col-sm-10">
<select name="braspag5_in" id="input-order-status" class="form-control">
<?php foreach ($order_statuses as $order_status) { ?>
<?php if ($order_status['order_status_id'] == $braspag5_in) { ?>
<option value="<?php echo $order_status['order_status_id']; ?>" selected="selected"><?php echo $order_status['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $order_status['order_status_id']; ?>"><?php echo $order_status['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-pending-status">Pago</label>
<div class="col-sm-10">
<select name="braspag5_pg" id="input-pending-status" class="form-control">
<?php foreach ($order_statuses as $order_status) { ?>
<?php if ($order_status['order_status_id'] == $braspag5_pg) { ?>
<option value="<?php echo $order_status['order_status_id']; ?>" selected="selected"><?php echo $order_status['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $order_status['order_status_id']; ?>"><?php echo $order_status['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-failed-status">Negado</label>
<div class="col-sm-10">
<select name="braspag5_ne" id="input-failed-status" class="form-control">
<?php foreach ($order_statuses as $order_status) { ?>
<?php if ($order_status['order_status_id'] == $braspag5_ne) { ?>
<option value="<?php echo $order_status['order_status_id']; ?>" selected="selected"><?php echo $order_status['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $order_status['order_status_id']; ?>"><?php echo $order_status['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>
<div class="form-group">
<label class="col-sm-2 control-label" for="input-chargeback-status">Autorizado</label>
<div class="col-sm-10">
<select name="braspag5_au" id="input-chargeback-status" class="form-control">
<?php foreach ($order_statuses as $order_status) { ?>
<?php if ($order_status['order_status_id'] == $braspag5_au) { ?>
<option value="<?php echo $order_status['order_status_id']; ?>" selected="selected"><?php echo $order_status['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $order_status['order_status_id']; ?>"><?php echo $order_status['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-chargeback-status">Pendente</label>
<div class="col-sm-10">
<select name="braspag5_pe" id="input-chargeback-status" class="form-control">
<?php foreach ($order_statuses as $order_status) { ?>
<?php if ($order_status['order_status_id'] == $braspag5_pe) { ?>
<option value="<?php echo $order_status['order_status_id']; ?>" selected="selected"><?php echo $order_status['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $order_status['order_status_id']; ?>"><?php echo $order_status['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label" for="input-chargeback-status">Cancelado</label>
<div class="col-sm-10">
<select name="braspag5_ca" id="input-chargeback-status" class="form-control">
<?php foreach ($order_statuses as $order_status) { ?>
<?php if ($order_status['order_status_id'] == $braspag5_ca) { ?>
<option value="<?php echo $order_status['order_status_id']; ?>" selected="selected"><?php echo $order_status['name']; ?></option>
<?php } else { ?>
<option value="<?php echo $order_status['order_status_id']; ?>"><?php echo $order_status['name']; ?></option>
<?php } ?>
<?php } ?>
</select>
</div>
</div>
</div>

<?php } ?>

<div role="tabpanel" class="tab-pane" id="pedidos">
<table id="example-cielo" class="display table table-bordered" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th>ID</th>
                <th>Pedido</th>
                <th>Total</th>
                <th>&Agrave; Pagar</th>
                <th>TID</th>
                <th>Tipo</th>
				<th>Bandeira</th>
				<th>Status Atual</th>
				<th>LR</th>
				<th>BIN</th>
				<th></th>
            </tr>
        </thead>
        <tbody>
		
		<?php
		if(isset($pedidos_cielo->rows)){
		foreach($pedidos_cielo->rows AS $k=>$v){
		?>
            <tr>
                <td><?php echo $v['id_braspag'];?></td>
                <td><a href="index.php?route=sale/order/info&token=<?php echo $token;?>&order_id=<?php echo $v['id_pedido'];?>"><?php echo $v['id_pedido'];?></a></td>
                <td><?php echo $v['total_pedido'];?></td>
                <td><?php echo $v['total_pago'];?></td>
                <td><?php echo $v['tid'];?></td>
				<td><?php echo strtoupper($v['tipo']);?></td>
                <td><?php echo strtoupper($v['bandeira']);?> em <?php echo $v['parcelas'];?>x</td>
				<td><?php echo getStatusCielo5($v['status']);?></td>
				<td><?php echo $v['lr'];?> <?php echo $v['lr_log'];?></td>
				<td><?php echo $v['bin'];?></td>
				<td><a class="btn btn-info btn-xs" onclick="javascript:abrir('index.php?route=payment/braspag5/consultar&token=<?php echo $token;?>&pedido=<?php echo $v['id_pedido'];?>&tid=<?php echo $v['link'];?>');">Detalhes</a></td>
            </tr>
		<?php
		}
		}
		?>

        </tbody>
    </table>
</div>

<div role="tabpanel" class="tab-pane" id="sobre">
<img src="data:image/jpg;base64,"/>
<br><br>
<b>Vers&atilde;o:</b> 6.0<br>
<b>Data:</b> 01/08/2018<br>
<br>
Suporte: <a href="http://www.loja5.com.br/suporte/" target="_blank">http://www.loja5.com.br/suporte/</a><br>
Contato/Vendas: <u>suportedaloja@gmail.com</u><br>
</div>		

</div>

</form>

</div>
</div>
</div>
</div>
</div>

<script>
$('input[name="braspag5_soft"]').on( "keyup keydown", function() {
var string = $(this).val();
$(this).val(string.replace(/[^0-9a-zA-Z]/g,'').toUpperCase());
});
$(document).ready(function() {
	$('#tabelas a:first').tab('show');
    $('#example-cielo').DataTable({
		"bSort": false,
        "language": {
            "url": "//cdn.datatables.net/plug-ins/9dcbecd42ad/i18n/Portuguese.json"
        }
    });
} );
</script>
<script type="text/javascript">
<!--
function isNumberKey(evt){
          var charCode = (evt.which) ? evt.which : event.keyCode;
          if (charCode != 46 && charCode > 31 
            && (charCode < 48 || charCode > 57))
             return false;

          return true;
}
function abrir(URL) {
 
  var width = 600;
  var height = 500;
 
  var left = 99;
  var top = 99;
 
  window.open(URL,'janela', 'width='+width+', height='+height+', top='+top+', left='+left+', scrollbars=yes, status=no, toolbar=no, location=no, directories=no, menubar=no, resizable=no, fullscreen=no');
 
}
//-->
</script>


        <script type="text/javascript"><!--
        function save(type){
            var input = document.createElement('input');
            input.type = 'hidden';
            input.name = 'button';
            input.value = type;
            form = $("form[id^='form-']").append(input);
            form.submit();
        }
        //--></script>
            
<?php echo $footer; ?> 