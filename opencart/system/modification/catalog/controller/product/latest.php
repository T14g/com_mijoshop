...
class ControllerProductLatest extends Controller {
	...
	public function index() { 
		...
		$data['limits'][] = array(
			...
			'href'  => $this->url->link('product/latest', $url . '&limit=' . $this->config->get('config_product_limit'))
		);
		...
		if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/product/latest.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/product/latest.tpl', $data));
        }
        else if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/latest.tpl')) {
