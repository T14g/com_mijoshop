...
class ControllerProductCategory extends Controller {
 ...
 //roberto
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
                ...
                #mijoshop-start
                $category_name_len = strlen($result['name']);

                $category_name = $result['name'];

                if ($this->config->get('config_category_name_limit')) {
                    $name = utf8_substr($category_name, 0, $this->config->get('config_category_name_limit'));

                    $result['name'] = $name;

                    $result['name'] .= ($category_name_len > $this->config->get('config_category_name_limit')) ? '..' : '' ;
                }
                #mijoshop-end
 ...
 //roberto
			...
			if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/product/category.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/product/category.tpl', $data));
        }
