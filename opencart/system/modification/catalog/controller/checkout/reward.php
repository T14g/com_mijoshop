...
class ControllerCheckoutReward extends Controller {
	public function index() {
		...
		if ($points && $points_total && $this->config->get('reward_status')) {
			...
			if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/checkout/reward.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/checkout/reward.tpl', $data));
        }
        else if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/reward.tpl')) {
