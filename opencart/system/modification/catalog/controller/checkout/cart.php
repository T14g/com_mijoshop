...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            $Itemid = $this->model_checkout_order->getItemid('cart');
        
            if (!empty($Itemid)) {
                $data['action'] = $this->url->link('checkout/cart/edit', 'Itemid=' . $Itemid);
            } 
            #mijoshop-finish
			...
			if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/checkout/cart.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/checkout/cart.tpl', $data));
        }
...
$this->cart->clear();
		...
		if ($product_info) {
			if (isset($this->request->post['quantity'])) {
				...
				$quantity = 1;
			            ...
			            if (isset($this->request->post['option_oc'])) { #mijoshop
				                $option = array_filter($this->request->post['option_oc']); #mijoshop
			} else {
				$option = array();
			}
            ...
            $json['location'] = $this->url->link('checkout/cart', '', 'SSL'); #mijoshop
				$json['total'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total));
			} else {
    ...
    public function addExternal(){
		...
		$this->response->redirect(MijoShop::get('router')->route('index.php?route=checkout/cart'));
    }
