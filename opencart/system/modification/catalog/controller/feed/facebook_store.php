...
class ControllerFeedFacebookStore extends Controller {
	public function index() {
        if ($this->config->get('facebook_store_status')) {
            ...
            if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/feed/facebook_store.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/feed/facebook_store.tpl', $data));
        }
        else if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/feed/facebook_store.tpl')) {
