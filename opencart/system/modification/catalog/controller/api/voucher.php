...
class ControllerApiVoucher extends Controller {
	...
	public function add() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (isset($this->request->post['from_email']) && (utf8_strlen($this->request->post['from_email']) > 96) || !preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $this->request->post['from_email'])) {
			...
			if (isset($this->request->post['to_email']) && (utf8_strlen($this->request->post['to_email']) > 96) || !preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $this->request->post['to_email'])) {
