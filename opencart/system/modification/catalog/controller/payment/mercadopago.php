...
include_once JPATH_MIJOSHOP_OC."/catalog/controller/payment/mercadopago_include.php";
...
class ControllerPaymentmercadopago extends Controller {
	...
	public function index() {
	   ...
	   switch($order_info['currency_code']) {
		...
		//set back url
		$back_urls = array(
		    "pending" => $url . '/index.php?option=com_mijoshop&route=payment/mercadopago/callback',
		    "success" => $url . '/index.php?option=com_mijoshop&route=payment/mercadopago/callback'
		);
		...
		if($preferenceResult['status'] == 201):
			...
			if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/payment/mercadopago.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/payment/mercadopago.tpl', $data));
        }
        else if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/mercadopago.tpl')) {
	...
	public function callback() {
	      $this->response->redirect(HTTP_SERVER . 'index.php?option=com_mijoshop&route=checkout/success');
	}
