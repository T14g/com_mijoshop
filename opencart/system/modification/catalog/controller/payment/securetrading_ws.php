...
class ControllerPaymentSecureTradingWs extends Controller {
	...
	public function index() {
		...
		if ($order_info) {
			...
			if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/payment/securetrading_ws.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/payment/securetrading_ws.tpl', $data));
        }
        else if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/securetrading_ws.tpl')) {
