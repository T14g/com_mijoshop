...
class ControllerPaymentAuthorizeNetSim extends Controller {
	public function index() {
		...
		$data['x_relay_url'] = $this->url->link('payment/authorizenet_sim/callback', '', 'SSL');
        ...
        if ($this->config->get('authorizenet_sim_mode')) {
            $data['action'] = 'https://test.authorize.net/gateway/transact.dll';
        } else {
            //$data['action'] = 'https://secure.authorize.net/gateway/transact.dll';
            $data['action'] = 'https://secure2.authorize.net/gateway/transact.dll';
        }

		if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/payment/authorizenet_sim.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/payment/authorizenet_sim.tpl', $data));
        }
        else if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/authorizenet_sim.tpl')) {
