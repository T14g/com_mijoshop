...
class ControllerPaymentAlphauserpoints extends Controller {
	...
	public function index() {
		...
		if (file_exists(JPATH_ROOT.'/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/payment/alphauserpoints.tpl')) {
            $this->response->setOutput($this->load->view('/templates/'.MijoShop::getTmpl().'/html/com_mijoshop/payment/alphauserpoints.tpl', $data));
        }
        else if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/alphauserpoints.tpl')) {
	...
	public function confirm() {
		...
		if ($this->model_payment_alphauserpoints->getAUPPoints($this->request->post['user_id']) >= $priceInPoints)	{
			$description = 'MijoShop Order #' . $this->session->data['order_id'];
