...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
        ...
        MijoShop::get('base')->trigger('onMijoshopBeforeOrderSave', array(&$data, &$data['products'], &$data['vouchers'], &$data['totals'], true)); #mijoshop-start
        ...
        MijoShop::get('base')->trigger('onMijoshopAfterOrderSave', array(&$data, &$data['products'], &$data['vouchers'], &$data['totals'], &$order_id, true)); #mijoshop-start
    ...
    public function getItemid($view) {
        ...
        $itemid = $this->db->query("SELECT id FROM #__menu WHERE `link`= 'index.php?option=com_mijoshop&view=" . $view . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
        ...
        MijoShop::get('base')->trigger('onMijoshopBeforeOrderStatusUpdate', array(&$order_info, &$order_id, &$order_status_id, &$notify)); #mijoshop-start

		if ($order_info) {
      $this->load->model('account/custom_field');      $custom_fields = $this->model_account_custom_field->getCustomFields();      
			// Fraud Detection
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['logo'] = $this->config->get('config_url') . 'components/com_mijoshop/opencart/image/' . $this->config->get('config_logo'); #mijoshop-start
				$data['store_name'] = $order_info['store_name'];
				$data['store_url'] = $order_info['store_url'];
				$data['customer_id'] = $order_info['customer_id'];
				$data['link'] = $order_info['store_url'] . 'index.php?option=com_mijoshop&route=account/order/info&order_id=' . $order_id;

				if ($download_status) {
					$data['download'] = $order_info['store_url'] . 'index.php?option=com_mijoshop&route=account/download';
				} else {
					$data['download'] = '';
				}
      ...
      foreach ($custom_fields as $custom_field) {          if (isset($order_info['payment_custom_field'][$custom_field['custom_field_id']])) {              $payment_custom_field_value = $order_info['payment_custom_field'][$custom_field['custom_field_id']];          }          if (isset($order_info['custom_field'][$custom_field['custom_field_id']])) {              $payment_custom_field_value = $order_info['custom_field'][$custom_field['custom_field_id']];          }          if (isset($payment_custom_field_value)) {              if ($custom_field['type'] == 'select' || $custom_field['type'] == 'radio' || $custom_field['type'] == 'checkbox') {                  foreach ($custom_field['custom_field_value'] as $custom_field_option) {                      if ($custom_field_option['custom_field_value_id'] == $payment_custom_field_value) {                          $payment_custom_field_value = $custom_field_option['name'];                      }                  }              }              $format = str_replace('{custom_field_' . $custom_field['custom_field_id'] . '}', $payment_custom_field_value, $format);          }      }      $format = preg_replace('/(({custom_field(.*?)?}))/sim', '', $format);      
      ...
      foreach ($custom_fields as $custom_field) {          if (isset($order_info['shipping_custom_field'][$custom_field['custom_field_id']])) {              $shipping_custom_field_value = $order_info['shipping_custom_field'][$custom_field['custom_field_id']];          }          if (isset($order_info['custom_field'][$custom_field['custom_field_id']])) {              $shipping_custom_field_value = $order_info['custom_field'][$custom_field['custom_field_id']];          }          if (isset($shipping_custom_field_value)) {              if ($custom_field['type'] == 'select' || $custom_field['type'] == 'radio' || $custom_field['type'] == 'checkbox') {                  foreach ($custom_field['custom_field_value'] as $custom_field_option) {                      if ($custom_field_option['custom_field_value_id'] == $shipping_custom_field_value) {                          $shipping_custom_field_value = $custom_field_option['name'];                      }                  }              }              $format = str_replace('{custom_field_' . $custom_field['custom_field_id'] . '}', $shipping_custom_field_value, $format);          }      }      $format = preg_replace('/(({custom_field(.*?)?}))/sim', '', $format);      
      ...
      foreach ($custom_fields as $custom_field) {          if (isset($order_info['shipping_custom_field'][$custom_field['custom_field_id']])) {              $shipping_custom_field_value = $order_info['shipping_custom_field'][$custom_field['custom_field_id']];          }          if (isset($order_info['custom_field'][$custom_field['custom_field_id']])) {              $shipping_custom_field_value = $order_info['custom_field'][$custom_field['custom_field_id']];          }          if (isset($shipping_custom_field_value)) {              if ($custom_field['type'] == 'select' || $custom_field['type'] == 'radio' || $custom_field['type'] == 'checkbox') {                  foreach ($custom_field['custom_field_value'] as $custom_field_option) {                      if ($custom_field_option['custom_field_value_id'] == $shipping_custom_field_value) {                          $shipping_custom_field_value = $custom_field_option['name'];                      }                  }              }              $format = str_replace('{custom_field_' . $custom_field['custom_field_id'] . '}', $shipping_custom_field_value, $format);          }      }      $format = preg_replace('/(({custom_field(.*?)?}))/sim', '', $format);      
			...
			$data['shipping_address'] = false;
				...
				if (!$this->model_payment_amazon_checkout->isAmazonOrder($order_info['order_id'])) {
					...
					if ($order_info['customer_id']) {
						...
						$text .= $order_info['store_url'] . 'index.php?option=com_mijoshop&route=account/order/info&order_id=' . $order_id . "\n\n";
					}

					if ($download_status) {
						$text .= $language->get('text_new_download') . "\n";
						$text .= $order_info['store_url'] . 'index.php?option=com_mijoshop&route=account/download' . "\n\n";
					}
			...
			if ($order_info['order_status_id'] && $order_status_id) {
				...
				//roberto
				$msgEmail ='';
				$produtos = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product AS op INNER JOIN ". DB_PREFIX ."product_to_category AS pc ON op.product_id = pc.product_id WHERE order_id = '" . (int)$order_id . "'");

				if ($order_status_id == 5) {					
					foreach ($produtos->rows as $product) {
						$categoriaSemi = $product['category_id'];
					}
					if ($categoriaSemi == 93) {
						$msgEmail = '
						<p>Bem Vindo(a) ao ACLS Semipresencial<br/>
						Saiba que é uma satisfação iniciar este processo com você.</p>
						<p><img src="https://www.ctsem.com/passo-passo.jpg" alt="ACLS" /></p>
						<p>Agora que se inscreveu na Parte Prática – CTSEM, você precisa se matricular e concluir sua certificação na AHA e somente após esta aprovação você poderá agendar a parte PRÁTICA. </p>
						<p>
						<a href="https://www.elearning.heart.org/course/20" >			
						<img src="https://www.ctsem.com/email-online.jpg" alt="ACLS Semipresencial" />
						</a>
						</p>
						<p>Link para inscrição: https://www.elearning.heart.org/course/20 <br/>
						Após concluir a parte on-line deste curso, você deve concluir a parte prática no CTSEM, para obter mais detalhes ou para agendar sua aula prática confira a agenda de cursos https://www.ctsem.com/cursos/acls, e envie um e-mail para secretaria@ctsem.com para reservar sua vaga.
						</p>
						';
						//$message .= $msgEmail;
					}
				...
				if ($order_info['customer_id']) {
					...
					$message .= $order_info['store_url'] . 'index.php?option=com_mijoshop&route=account/order/info&order_id=' . $order_id . "\n\n";
				}
        ...
        MijoShop::get('base')->trigger('onMijoshopAfterOrderStatusUpdate', array(&$order_info, &$order_id, &$order_status_id, &$notify)); #mijoshop-start
	}
}