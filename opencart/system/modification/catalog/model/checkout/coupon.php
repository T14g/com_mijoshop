...
class ModelCheckoutCoupon extends Model {
	public function getCoupon($code, $verify = 1) {
		$status = true;

		$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code = '" . $this->db->escape($code) . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1'");

		if ($coupon_query->num_rows) {
			if ($verify && $coupon_query->row['total'] > $this->cart->getSubTotal()) {
				$status = false;
			}
			...
			if ($verify && $coupon_query->row['uses_total'] > 0 && ($coupon_history_query->row['total'] >= $coupon_query->row['uses_total'])) {
				$status = false;
			}

			if ($verify && $coupon_query->row['logged'] && !$this->customer->getId()) {
				$status = false;
			}

			if ($verify && $this->customer->getId()) {
