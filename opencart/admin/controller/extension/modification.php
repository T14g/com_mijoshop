...
class ControllerExtensionModification extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validate()) {
			foreach ($this->request->post['selected'] as $modification_id) {
                $code = explode('.', $modification_id);
                
                if (!empty($code[0])) {
                    $this->model_extension_modification->deleteModificationByCode($code[0]);
                } else {
                    $this->model_extension_modification->deleteModification($modification_id);
                }
                
                #mijoshop
                if(file_exists(DIR_SYSTEM . 'xmls/' . $modification_id)) {
                    unlink(DIR_SYSTEM . 'xmls/' . $modification_id);
                } else if(file_exists(JPATH_MIJOSHOP_OC . '/vqmod/xml/' . $modification_id)){
                    unlink(JPATH_MIJOSHOP_OC . '/vqmod/xml/' . $modification_id);
                }
                #mijoshop
			}
	...
	public function enable() {
		...
		if (isset($this->request->get['modification_id']) && $this->validate()) {
            $code = explode('.', $this->request->get['modification_id']);
                
            if (!empty($code[0])) {
                $this->model_extension_modification->enableModificationByCode($code[0]);
            } else {
                $this->model_extension_modification->enableModification($this->request->get['modification_id']);
            }

            #mijoshop
            if(file_exists(DIR_SYSTEM . 'xmls/' . $this->request->get['modification_id'])) {
                rename(DIR_SYSTEM . 'xmls/' . $this->request->get['modification_id'], DIR_SYSTEM . 'xmls/' . str_replace('.xml_', '.xml', $this->request->get['modification_id']));
			} else if(file_exists(JPATH_MIJOSHOP_OC . '/vqmod/xml/' . $this->request->get['modification_id'])){
                rename(JPATH_MIJOSHOP_OC . '/vqmod/xml/' . $this->request->get['modification_id'], JPATH_MIJOSHOP_OC . '/vqmod/xml/' . str_replace('.xml_', '.xml', $this->request->get['modification_id']));
            }
            #mijoshop
	...
	public function disable() {
		...
		if (isset($this->request->get['modification_id']) && $this->validate()) {
            ...
            if (!empty($code[0])) {
                $this->model_extension_modification->disableModificationByCode($code[0]);
            } else {
            ...
            #mijoshop
            if(file_exists(DIR_SYSTEM . 'xmls/' . $this->request->get['modification_id'])) {
                rename(DIR_SYSTEM . 'xmls/' . $this->request->get['modification_id'], DIR_SYSTEM . 'xmls/' . str_replace('.xml', '.xml_', $this->request->get['modification_id']));
            } else if(file_exists(JPATH_MIJOSHOP_OC . '/vqmod/xml/' . $this->request->get['modification_id'])) {
                rename(JPATH_MIJOSHOP_OC . '/vqmod/xml/' . $this->request->get['modification_id'], JPATH_MIJOSHOP_OC . '/vqmod/xml/' . str_replace('.xml', '.xml_', $this->request->get['modification_id']));
            }
            #mijoshop
	...
	protected function getList() {
        ...
        $unvisible_xmls = array(
            'mijoshop_admin.xml',
            'mijoshop_admin_reports_graph.xml',
            'mijoshop_buttons.xml',
            'mijoshop_catalog.xml',
            'mijoshop_filters.xml',
            'mijoshop_j_integration.xml',
            'mijoshop_system.xml',
            'mijoshop_triggers.xml',
            'x_mijoshop_finish.xml',
            'xx_miwoshop_wordpress',
            'mijoshop_module_random_product.xml',
            'mijoshop_payment_paypal_standart_billing_detail.xml'
        );
        ...
        foreach($files as $file) {
            ...
            // Do not show core MijoShop xml files to users
            if(in_array($file_name, $unvisible_xmls)) {
                continue;
            }
	...
	protected function _checkVqmodfile() {
        $files_enable  = glob(JPATH_MIJOSHOP_OC . '/vqmod/xml/*.xml');
        $files_disable = glob(JPATH_MIJOSHOP_OC . '/vqmod/xml/*.xml_');
