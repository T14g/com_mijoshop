...
class ControllerExtensionInstaller extends Controller {
	...
	public function upload() {
		...
		if (!$json) {
			if (!empty($this->request->files['file']['name'])) {
				if ((strrchr($this->request->files['file']['name'], '.') != '.zip' && strrchr($this->request->files['file']['name'], '.') != '.xml') && (substr($this->request->files['file']['name'], -10) != '.ocmod.zip' && substr($this->request->files['file']['name'], -10) != '.ocmod.xml')) { #mijoshop-start
		...
		if (!$json) {
			...
			if (strrchr($this->request->files['file']['name'], '.') == '.xml') {
                ...
                if(empty($code)) {
                    ...
                    $_file = JPATH_MIJOSHOP_OC . '/vqmod/xml/' . $this->request->files['file']['name'];
				...
				if (file_exists($file)) {
					$json['step'][] = array(
						...
						'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/miwi_xml', 'token=' . $this->session->data['token'], 'SSL')),
						'path' => $path
					);
				...
				if (file_exists($file)) {
					...
					if ($zip) {
						...
						$json['step'][] = array(
							...
							'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/miwi_ftp', 'token=' . $this->session->data['token'], 'SSL')),
							'path' => $path
						);
						...
						while ($entry = zip_read($zip)) {
							...
							if (substr($zip_name, 0, 11) == 'install.sql') {
								$json['step'][] = array(
									...
									'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/miwi_sql', 'token=' . $this->session->data['token'], 'SSL')),
									'path' => $path
								);
							}
							...
							} else if (substr($zip_name, -4) == '.xml'){ #mijoshop start
                                $_file = JPATH_MIJOSHOP_OC . '/vqmod/xml/' . $zip_name;
							...
							// Compare admin files
							$file = JPATH_MIJOSHOP_OC .'/'. substr($zip_name,7); #mijoshop-start
							if (is_file($file) && strtolower(substr($zip_name, 0, 13)) == 'upload/admin/'){ #mijoshop-start
							...
							if (is_file($file) && strtolower(substr($zip_name, 0, 15)) == 'upload/catalog/') { #mijoshop-start
								$json['overwrite'][] = substr($zip_name, 7);
							}

							// Compare image files
							$file = JPATH_MIJOSHOP_OC .'/'. substr($zip_name,7); #mijoshop-start
							if (is_file($file) && strtolower(substr($zip_name, 0, 13)) == 'upload/image/') { #mijoshop-start
								$json['overwrite'][] = substr($zip_name, 7);
							}

							// Compare system files
							$file = JPATH_MIJOSHOP_OC .'/'. substr($zip_name, 7);
	...
	public function miwi_ftp() {
		...
		if (!$json) {
			...
			#mijoshop-start
			$root = JPATH_ROOT.'/components/com_mijoshop/opencart/';
    ...
    if ($source == 'admin' || $source == 'site') {
        ...
        $replace_output['JText'] 		               	= 'MText';
        $replace_output['COM_MIJOSHOP'] 		        = 'COM_MIWOSHOP';
	...
	public function miwi_xml() {
		$this->load->language('extension/installer');

        $isVqmod = 0; #mijoshop start
		$json = array();
		...
		if (!$json) {
            $json['overwrite'] = array(); #mijoshop-start
			...
			if ($xml) {
				try {
					...
					if(!empty($code)) {
						if ($code) {
							...
							if ($modification_info) {
								$json['overwrite'][] =  $code . '.xml'; #mijoshop-start
                    ...
                    #Xml replace text '', Because It doesn't necessary MijoShop
                    $xml = ''; #mijoshop-start
                    ...
                    if($isVqmod) {
                        $file  = DIR_UPLOAD . str_replace(array('../', '..\\', '..'), '', $this->request->post['path']) . '/' . $this->session->data['vqmod_file_name'];
						$msmod = JPATH_MIJOSHOP_OC.'/vqmod/xml/'. str_replace("upload/vqmod/xml/","",$this->session->data['vqmod_file_name']);
                    } else {
