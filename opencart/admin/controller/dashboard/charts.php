...
class ControllerDashboardCharts extends Controller {
	public function index() {
        ...
        $data['text_mijoshop_info'] = $this->language->get('text_mijoshop_info');
        ...
        $data['link_affiliates'] = $this->url->link('marketing/affiliate', 'token=' . $this->session->data['token'], 'SSL');
        $data['link_affiliate_waiting'] = $this->url->link('marketing/affiliate', 'token=' . $this->session->data['token'] . '&filter_approved=0', 'SSL');
    ...
    public function getChartData($modelFunction, $currency_format = false ) {
        ...
        $json   = array();
        $start  = JRequest::getCmd('start', '');
        $end    = JRequest::getCmd('end', '');
