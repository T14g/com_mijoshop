...
class ModelToolRedshop extends Model {
  	...
  	public function importCategories($post){
		...
		foreach($cats as $cat) {
			...
			$cat_image = empty($cat['category_thumb_image']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($cat['category_thumb_image']) : 'catalog/'.$db->getEscaped($cat['category_thumb_image']));
			...
			$q = "INSERT IGNORE INTO `#__mijoshop_category` ( `category_id` , `image` , `parent_id` , `sort_order` , `date_added` , `date_modified` , `status`) VALUES ('". $cat['category_id']."', '".$cat_image."', '".$cat['category_parent_id']."', '".$cat['ordering']."', '".$date."', '".$date."', '".$cat['published']."')";
			$db->setQuery($q);
			$db->query();
			
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ('". $cat['category_id']."', '".$shoplang['language_id']."', '".$cat_name."', '".$cat_desc."', '". $cat_metadesc."', '". $cat_metakey."')";
				$db->setQuery($q);
				$db->query();
			}
    ...
    private function _getPath($cat_id, $path = array()){
  	...
  	public function importProducts($post) {
		...
		foreach($pros as $pro){
            ...
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_product_description` (`product_id` , `language_id` , `name` , `description` , `meta_description` , `meta_keyword`) VALUES ('".$pro['product_id']."' , '".$shoplang['language_id']."' , '".$pro_name."' , '".$pro_desc."' , '". $pro_metadesc."', '". $pro_metakey."')";
				$db->setQuery($q);
				$db->query();
			}
			...
			$q = "SELECT * FROM #__redshop_media WHERE media_section = 'product' AND media_type = 'images' AND section_id = '".$pro['product_id']."' AND published= '1'";
			$db->setQuery($q);
			$p_imgs = $db->loadAssocList();
			
			foreach($p_imgs as $p_img){
				$img_name = ($jstatus) ? 'catalog/'.$db->escape($p_img['media_name']) : 'catalog/'.$db->getEscaped($p_img['media_name']);
				$q = "INSERT IGNORE INTO `#__mijoshop_product_image` (`product_image_id` , `product_id` , `image` , `sort_order`) VALUES ('".$p_img['media_id']."' , '".$p_img['section_id']."' , '".$img_name."' , '".$p_img['ordering']."')";
				$db->setQuery($q);
				$db->query();
			}
		...
		if (!empty($relresults)) {
			foreach($relresults as $relres) {
				$ptjc = "INSERT IGNORE INTO `#__mijoshop_product_related` (`related_id`, `product_id`) VALUES ('".$relres['related_id']."', '".$relres['product_id']."')";
				$db->setQuery($ptjc);
				$db->query();
			}
		}
	...
	public function importManufacturers($post) {
		...
		foreach($mans as $man) {
			...
			$man_image = empty($file['media_name']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($file['media_name']) : 'catalog/'.$db->getEscaped($file['media_name']));
			$man_name = ($jstatus) ? $db->escape(htmlspecialchars($man['manufacturer_name'])) : $db->getEscaped(htmlspecialchars($man['manufacturer_name']));
			
			$q = "INSERT IGNORE INTO `#__mijoshop_manufacturer` (`manufacturer_id`, `name`, `image`, `sort_order`) VALUES ('".$man['manufacturer_id']."', '".$man_name."', '".$man_image."', '".$man['ordering']."')";
			$db->setQuery($q);
			$db->query();
	...
	public function importUserInformation($post) {
		...
		foreach($users as $user) {
            ...
            if (!empty($address_id)) {
                ...
                $q = "INSERT IGNORE INTO #__mijoshop_address SET customer_id = '{$oc_customer_id}', firstname = '{$user['firstname']}', lastname = '{$user['lastname']}', address_1 = '{$user['address']}', city = '{$user['city']}', postcode = '{$user['zipcode']}', country_id = '{$country_id}', zone_id = '{$zone_id}'";
            }
			$db->setQuery($q);
			$db->query();
