...
class ModelToolTienda extends Model {
  	...
  	public function importCategories($post){
		...
		foreach($cats as $cat) {
			...
			$q = "INSERT IGNORE INTO `#__mijoshop_category` ( `category_id` , `image` , `parent_id` , `sort_order` , `date_added` , `date_modified` , `status`) VALUES ('". $cat['category_id']."', '".$cat_image."', '".$cat['parent_id']."', '".$cat['ordering']."', '".$datec."', '".$datem."', '".$cat['category_enabled']."')";
			$db->setQuery($q);
			$db->query();
			
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_category_description` (`category_id`, `language_id`, `name`, `description`) VALUES ('". $cat['category_id']."', '".$shoplang['language_id']."', '".$cat_name."', '".$cat_desc."')";
				$db->setQuery($q);
				$db->query();
			}
		...
		if (!empty($categories)){
			foreach($categories as $category){
				...
				foreach($path as $key => $_path){
					$db->setQuery("IGNORE INTO `#__mijoshop_category_path` (`category_id`, `path_id`, `level`) VALUES('{$category->category_id}','{$_path}','{$key}')");
					$db->query();
				}
			}
		}
    ...
    private function _getPath($cat_id, $path = array()){
  	...
  	public function importProducts($post) {
		...
		foreach($pros as $pro){
            ...
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_product_description` (`product_id` , `language_id` , `name` , `description`) VALUES ('".$pro['product_id']."' , '".$shoplang['language_id']."' , '".$pro_name."' , '".$pro_desc."')";
				$db->setQuery($q);
				$db->query();
			}
		...
		if (!empty($relresults)) {
			foreach($relresults as $relres) {
				$ptjc = "INSERT IGNORE INTO `#__mijoshop_product_related` (`related_id`, `product_id`) VALUES ('".$relres['product_id_to']."', '".$relres['product_id_from']."')";
				$db->setQuery($ptjc);
				$db->query();
			}
		}
