  	...
  	public function importCategories($post){
		...
		foreach($cats as $cat) {
            ...
            if (!empty($params['image'])) {
                ...
                if(!empty($imagges)){
                    $this->_copyFiles(JPATH_SITE.'/'.$imagges, $last);
                }
            }
            if($cat['parent_id'] == 1){
                $cat['parent_id'] = 0;
			...
			$datec = ($jstatus) ? JFactory::getDate($cat['created_time'])->toSql() : JFactory::getDate($cat['created_time'])->toMySQL();
			$datem = ($jstatus) ? JFactory::getDate($cat['modified_time'])->toSql() : JFactory::getDate($cat['modified_time'])->toMySQL();
			...
			$q = "INSERT IGNORE INTO `#__mijoshop_category` ( `category_id` , `image` , `parent_id` , `sort_order` , `date_added` , `date_modified` , `status`) VALUES ('". $cat['id']."', '".$cat_image."', '".$cat['parent_id']."', '0', '".$datec."', '".$datem."', '".$cat['published']."')";
			$db->setQuery($q);
			$db->query();
			
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ('". $cat['id']."', '".$shoplang['language_id']."', '".$cat_name."', '".$cat_desc."', '".$meta_desc."', '".$meta_key."')";
				$db->setQuery($q);
				$db->query();
			}
			
			$q = "INSERT IGNORE INTO `#__mijoshop_category_to_store` (`category_id` , `store_id`) VALUES ('".$cat['id']."' , '0')";
			$db->setQuery($q);
			$db->query();
		...
		foreach($pros AS $pro){
            ...
            for($i =0; $i < count($additional_image); $i++){
                if(isset($additional_image[$i]) && $additional_image[$i]){
                    ...
                    if(!empty($imagges)){
                        $this->_copyFiles(JPATH_SITE.'/'.$imagges, $extra_image);
                    }

                    $media_id = $i+1;
                    $pro_extra_image = (($jstatus) ? 'catalog/'.$db->escape($extra_image) : 'catalog/'.$db->getEscaped($extra_image));
                    $q = "INSERT IGNORE INTO `#__mijoshop_product_image` (`product_image_id` , `product_id` , `image` , `sort_order`) VALUES ('".$media_id."' , '".$pro['id']."' , '".$pro_extra_image."' , '".$i."')";
                    $db->setQuery($q);
                    $db->query();
                }
            }
			...
			$datec = ($jstatus) ? JFactory::getDate($pro['created'])->toSql() : JFactory::getDate($pro['created'])->toMySQL();
			$datem = ($jstatus) ? JFactory::getDate($pro['modified'])->toSql() : JFactory::getDate($pro['modified'])->toMySQL();
			$datea = ($jstatus) ? JFactory::getDate($pro['publish_up'])->toSql() : JFactory::getDate($pro['publish_up'])->toMySQL();
            ...
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_product_description` (`product_id` , `language_id` , `name` , `description`, `meta_description`, `meta_keyword`) VALUES ('".$pro['id']."','".$shoplang['language_id']."','".$pro_name."','".$pro_desc."','".$meta_desc."','".$meta_key."')";
				$db->setQuery($q);
				$db->query();
			}

			$q = "INSERT IGNORE INTO `#__mijoshop_product_to_store` (`product_id`, `store_id`) VALUES ('".$pro['id']."' , '0')";
			$db->setQuery($q);
			$db->query();

            $q = "INSERT IGNORE INTO `#__mijoshop_product_to_category` (`product_id`, `category_id`) VALUES ('".$pro['id']."', '".$pro['catid']."')";
            $db->setQuery($q);
            $db->query();
