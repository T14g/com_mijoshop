...
class ModelToolVirtuemart extends Model {
		...
		if (!empty($categories)){
			foreach($categories as $category){
				...
				foreach($path as $key => $_path){
					$db->setQuery("INSERT IGNORE INTO `#__mijoshop_category_path` (`category_id`, `path_id`, `level`) VALUES('{$category->category_child_id}','{$_path}','{$key}')");
					$db->query();
				}
			}
		}
    ...
    private function _getPath($cat_id, $path = array()){
  	...
  	public function importCategoriesToDB($cats, $oc_lang_id, $shoplangs) {
        ...
        $q = "SELECT name, language_id FROM #__mijoshop_language ORDER BY language_id";
        $db->setQuery($q);
        $mijoshop_langs = $db->loadAssocList();
		...
		foreach($cats as $cat) {
			...
			$cat_image = empty($cat['category_full_image']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($cat['category_full_image']) : 'catalog/'.$db->getEscaped($cat['category_full_image']));
			$cat_metadesc = empty($cat['metadesc']) ? '' : (($jstatus) ? $db->escape($cat['metadesc']) : $db->getEscaped($cat['metadesc']));
			$cat_metakey = empty($cat['metakey']) ? '' : (($jstatus) ? $db->escape($cat['metakey']) : $db->getEscaped($cat['metakey']));
			...
			$q = "INSERT IGNORE INTO `#__mijoshop_category` (`category_id`, `image`, `parent_id`, `sort_order`, `date_added`, `date_modified`, `status`) VALUES ('". $cat['category_id']."', '".$cat_image."', '".$cat['category_parent_id']."', '".$i."', '".$datec."', '".$datem."', '".$cat['published']."')";
			$db->setQuery($q);
			$db->query();
			
            foreach($mijoshop_langs AS $mijoshop_lang){
				$jpcat = "INSERT IGNORE INTO `#__mijoshop_category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ('". $cat['category_id']."', '".$mijoshop_lang['language_id']."', '".$cat_name."', '". $cat_desc."', '". $cat_metadesc."', '". $cat_metakey."')";
				$db->setQuery($jpcat);
				$db->query();
			}
					...
					if(!empty($joomlang)){
						...
						if(!empty($joomfishname)){
							if(!empty($existss)){
								...
								$q = "INSERT IGNORE INTO `#__mijoshop_category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ('".$cat['category_id']."' , '".$shoplang['language_id']."' , '".$joomfishname."' , '".$joomfishdesc."', '". $cat_metadesc."', '". $cat_metakey."')";
								$db->setQuery($q);
								$db->query();
							}
						}
  	...
  	public function importProducts($post) {
		...
		if (!empty($results)) {
			foreach($results as $ptcs) {
				$q = "INSERT IGNORE INTO `#__mijoshop_product_to_category` (`product_id`, `category_id`) VALUES ('".$ptcs['product_id']."', '".$ptcs['category_id']."');";
				$db->setQuery($q);
				$db->query();
			}
		}
  	...
  	public function importProductsToDB($pros, $oc_lang_id, $shoplangs) {
		...
		foreach($pros as $pro){
			if ($version == '1') {
				...
				$q = "SELECT m.file_url, pm.virtuemart_media_id, pm.ordering, COUNT(pm.virtuemart_media_id) AS imgcount FROM #__virtuemart_product_medias AS pm, #__virtuemart_medias AS m WHERE pm.virtuemart_media_id = m.virtuemart_media_id AND pm.virtuemart_product_id = ".$pro['product_id'];
				$db->setQuery($q);
				...
				foreach($pro_imagess as $pro_images){
					...
					if($pro_images['ordering'] == 0 || $pro_images['imgcount'] == 1){
						$pro['product_full_image'] = $extra_image;
					}
					else {
						$pro_image = (($jstatus) ? 'catalog/'.$db->escape($extra_image) : 'catalog/'.$db->getEscaped($extra_image));
						$q = "INSERT IGNORE INTO `#__mijoshop_product_image` (`product_image_id` , `product_id` , `image` , `sort_order`) VALUES ('".$pro_images['virtuemart_media_id']."' , '".$pro['product_id']."' , '".$pro_image."' , '".$pro_images['ordering']."')";
						$db->setQuery($q);
						$db->query();
					}
				}
			...
			$pro_image = empty($pro['product_full_image']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($pro['product_full_image']) : 'catalog/'.$db->getEscaped($pro['product_full_image']));
			$pro_metadesc = empty($pro['metadesc']) ? '' : (($jstatus) ? $db->escape($pro['metadesc']) : $db->getEscaped($pro['metadesc']));
			$pro_metakey = empty($pro['metakey']) ? '' : (($jstatus) ? $db->escape($pro['metakey']) : $db->getEscaped($pro['metakey']));
			...
			$product_sku = ($jstatus) ? $db->escape($pro['product_sku']) : $db->getEscaped($pro['product_sku']);
			$pro_price = ($jstatus) ? $db->escape($pro['product_price']) : $db->getEscaped($pro['product_price']);
			
			$q = "INSERT IGNORE INTO `#__mijoshop_product` (`product_id`, `model`, `sku`, `location`, `quantity`, `stock_status_id`, `image`, `manufacturer_id`, `shipping`, `price`, `tax_class_id`, `date_available`, `weight`, `weight_class_id`, `length`, `width`, `height`, `status`, `date_added`, `date_modified`, `viewed`) VALUES ('".$pro['product_id']."', '".$product_sku."', '".$product_sku."', '', '".$pro['product_in_stock']."', '7', '".$pro_image."', '".$man_id."', '1', '".$pro_price."', '', CURDATE(), '".$pro['product_weight']."', '0', '0.00', '0.00', '0.00',  '".$pro['published']."', '".$datec."', '".$datem."', '0')";
			$db->setQuery($q);
			$db->query();
			
            foreach($mijoshop_langs AS $mijoshop_lang){
				$q = "INSERT IGNORE INTO `#__mijoshop_product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ('".$pro['product_id']."' , '".$mijoshop_lang['language_id']."' , '".$pro_name."' , '".$pro_desc."', '". $pro_metadesc."', '". $pro_metakey."')";
				$db->setQuery($q);
				$db->query();
			}
			...
			if ($version == '2') {
				$q = "SELECT m.virtuemart_media_id, m.file_meta FROM #__virtuemart_product_medias AS pm, #__virtuemart_medias AS m WHERE pm.virtuemart_media_id = m.virtuemart_media_id AND m.file_type = 'product' AND pm.virtuemart_product_id='".$pro['product_id']."'";
			}
			else {
				$q = "SELECT file_id AS virtuemart_media_id, file_name AS file_meta FROM #__vm_product_files WHERE file_mimetype = 'image/jpeg' AND file_product_id='".$pro['product_id']."'";
			}
			$db->setQuery($q);
			$p_imgs = $db->loadAssocList();
			
			foreach($p_imgs as $p_img){
				if ($version == '2') {
					$img_name = ($jstatus) ? 'catalog/'.$db->escape($p_img['file_meta']) : 'catalog/'.$db->getEscaped($p_img['file_meta']);
				}
				else {
					$p_img['file_meta'] = str_replace('/components/com_virtuemart/shop_image/product/', '', $p_img['file_meta']);
					$img_name = ($jstatus) ? 'catalog/'.$db->escape($p_img['file_meta']) : 'catalog/'.$db->getEscaped($p_img['file_meta']);
				}
				$q = "INSERT IGNORE INTO `#__mijoshop_product_image` (`product_image_id` , `product_id` , `image` , `sort_order`) VALUES ('".$p_img['virtuemart_media_id']."' , '".$pro['product_id']."' , '".$img_name."' , '0')";
				$db->setQuery($q);
				$db->query();
			}
					...
					if(!empty($joomlang)){
						...
						if(!empty($joomfishname)){
							if(!empty($existss)){
								...
								$q = "INSERT IGNORE INTO `#__mijoshop_product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ('".$pro['product_id']."' , '".$shoplang['language_id']."' , '".$joomfishname."', '".$joomfishdescs."', '". $pro_metadesc."', '". $pro_metakey."')";
								$db->setQuery($q);
								$db->query();
							}
						}
	...
	public function importManufacturers($post) {
		...
		foreach($mans as $man) {
			...
			$man_image = empty($pro['image']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($man['image']) : 'catalog/'.$db->getEscaped($man['image']));
			$man_name = ($jstatus) ? $db->escape(htmlspecialchars($man['mf_name'])) : $db->getEscaped(htmlspecialchars($man['mf_name']));
	...
	public function importUsers($post) {
		...
		foreach($users as $user) {
			...
			$user_fname = ($jstatus) ? $db->escape(htmlspecialchars($user['first_name'])) : $db->getEscaped(htmlspecialchars($user['first_name']));
			$user_lname = ($jstatus) ? $db->escape(htmlspecialchars($user['last_name'])) : $db->getEscaped(htmlspecialchars($user['last_name']));
			$user_company = ($jstatus) ? $db->escape(htmlspecialchars($user['company'])) : $db->getEscaped(htmlspecialchars($user['company']));
