...
class ModelToolHikashop extends Model {
  	...
  	public function importCategories($post){
		...
		foreach($cats as $cat) {
			...
			$cat_image = empty($file['file_path']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($file['file_path']) : 'catalog/'.$db->getEscaped($file['file_path']));
			...
			$q = "INSERT IGNORE INTO `#__mijoshop_category` ( `category_id` , `image` , `parent_id` , `date_added` , `date_modified` , `status` , `sort_order`) VALUES ('". $cat['category_id']."', '".$cat_image."', '".$cat['category_parent_id']."', '".$datec."', '".$datem."', '".$cat['category_published']."', '".$cat['category_ordering']."' )";
			$db->setQuery($q);
			$db->query();
			
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_category_description` (`category_id` , `language_id` , `name` , `description` , `meta_description` , `meta_keyword`) VALUES ('". $cat['category_id']."', '".$shoplang['language_id']."', '".$cat_name."', '".$cat_desc."' , '".$meta_desc."' , '".$meta_keys."' )";
				$db->setQuery($q);
				$db->query();
			}
		...
		if (!empty($categories)){
			foreach($categories as $category){
				...
				foreach($path as $key => $_path){
					$db->setQuery("INSERT IGNORE INTO `#__mijoshop_category_path` (`category_id`, `path_id`, `level`) VALUES('{$category->category_id}','{$_path}','{$key}')");
					$db->query();
				}
			}
		}
    ...
    private function _getPath($cat_id, $path = array()){
  	...
  	public function importProducts($post) {
		...
		foreach($pros as $pro){
			...
			$pro_image = empty($file['file_path']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($file['file_path']) : 'catalog/'.$db->getEscaped($file['file_path']));
            ...
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_product_description` (`product_id` , `language_id` , `name` , `description` , `meta_description` , `meta_keyword`) VALUES ('".$pro['product_id']."' , '".$shoplang['language_id']."' , '".$pro_name."' , '".$pro_desc."' , '".$meta_desc."' , '".$meta_keys."')";
				$db->setQuery($q);
				$db->query();
			}
		...
		if (!empty($results)) {
			foreach($results as $ptrlt) {
				$ptjr = "INSERT IGNORE INTO `#__mijoshop_product_related` (`product_id`, `related_id`) VALUES ('".$ptrlt['product_id']."', '".$ptrlt['product_related_id']."');";
				$db->setQuery($ptjr);
				$db->query();
			}
		}
	...
	public function importManufacturers($post) {
		...
		foreach($mans as $man) {
			...
			$man_image = empty($file['file_path']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($file['file_path']) : 'catalog/'.$db->getEscaped($file['file_path']));
			$man_name = ($jstatus) ? $db->escape(htmlspecialchars($man['category_name'])) : $db->getEscaped(htmlspecialchars($man['category_name']));
			
			$q = "INSERT IGNORE INTO `#__mijoshop_manufacturer` (`manufacturer_id`, `name`, `image`) VALUES ('".$man['category_id']."', '".$man_name."', '".$man_image."');";
			$db->setQuery($q);
			$db->query();
			
			$q = "INSERT IGNORE INTO `#__mijoshop_manufacturer_to_store` (`manufacturer_id` , `store_id`) VALUES ('".$man['category_id']."' , '0')";
			$db->setQuery($q);
			$db->query();
    ...
    public function importUsers($post) {
        ...
        $query = "SELECT user_id, user_cms_id FROM `#__hikashop_user`";
        $db->setQuery($query);
        ...
        foreach($users as $user) {
            ...
            if(!empty($addresses)){
                foreach($addresses AS $address){
                    ...
                    $query = "SELECT zone_id FROM `#__mijoshop_zone` WHERE code = '".$zone['code3']."' AND country_id='".$oc_country."'";
                    $db->setQuery($query);
                    $oc_zone = $db->loadResult();

                    $user_fname = ($jstatus) ? $db->escape(htmlspecialchars($address['address_firstname'])) : $db->getEscaped(htmlspecialchars($address['address_firstname']));
                    $user_lname = ($jstatus) ? $db->escape(htmlspecialchars($address['address_lastname'])) : $db->getEscaped(htmlspecialchars($address['address_lastname']));
                    $user_company = ($jstatus) ? $db->escape(htmlspecialchars($address['address_company'])) : $db->getEscaped(htmlspecialchars($address['address_company']));

                    $query = "SELECT ocustomer_id FROM `#__mijoshop_juser_ocustomer_map` WHERE juser_id = '".$user['user_cms_id']."'";
                    $db->setQuery($query);
                ...
                echo 'There is no address for user ID <i> '. $user['user_cms_id'] .'</i>.<br />';
            }
        }
