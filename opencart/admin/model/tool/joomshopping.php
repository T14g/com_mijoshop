...
class ModelToolJoomshopping extends Model {
  	...
  	public function importCategories($post){
		...
		foreach($cats as $cat) {
			...
			$q = "INSERT IGNORE INTO `#__mijoshop_category` ( `category_id` , `image` , `parent_id` , `sort_order` , `date_added` , `date_modified` , `status`) VALUES ('". $cat['category_id']."', '".$cat_image."', '".$cat['category_parent_id']."', '".$cat['ordering']."', '".$datec."', '".$datec."', '".$cat['category_publish']."')";
			$db->setQuery($q);
			$db->query();
			
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ('". $cat['category_id']."', '".$shoplang['language_id']."', '".$cat_name."', '".$cat['category_description']."', '".$cat['meta_desc']."', '".$cat['meta_key']."')";
				$db->setQuery($q);
				$db->query();
			}
    ...
    private function _getPath($cat_id, $path = array()){
  	...
  	public function importProducts($post) {
		...
		foreach($pros AS $pro){
			...
			$pro_image = empty($pro['image']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($pro['image']) : 'catalog/'.$db->getEscaped($pro['image']));
            ...
            foreach($shoplangs AS $shoplang){
				$q = "INSERT IGNORE INTO `#__mijoshop_product_description` (`product_id` , `language_id` , `name` , `description`, `meta_description`, `meta_keyword`) VALUES ('".$pro['product_id']."','".$shoplang['language_id']."','".$pro_name."','".$pro_desc."','".$pro['meta_desc']."','".$pro['meta_key']."')";
				$db->setQuery($q);
				$db->query();
			}
		...
		if (!empty($relresults)) {
			foreach($relresults as $relres) {
				$ptjc = "INSERT IGNORE INTO `#__mijoshop_product_related` (`related_id`, `product_id`) VALUES ('".$relres['product_related_id']."', '".$relres['product_id']."')";
				$db->setQuery($ptjc);
				$db->query();
			}
		}
	...
	public function importManufacturers($post) {
		...
		foreach($mans as $man) {
			$man_image = empty($man['manufacturer_logo']) ? '' : (($jstatus) ? 'catalog/'.$db->escape($man['manufacturer_logo']) : 'catalog/'.$db->getEscaped($man['manufacturer_logo']));
