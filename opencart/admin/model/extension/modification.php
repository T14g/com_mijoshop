...
class ModelExtensionModification extends Model {
    ...
    public function enableModificationByCode($code) {
		$this->db->query("UPDATE " . DB_PREFIX . "modification SET status = '1' WHERE code = '" . $this->db->escape($code) . "'");
	}
	...
	public function disableModificationByCode($code) {
		$this->db->query("UPDATE " . DB_PREFIX . "modification SET status = '0' WHERE code = '" . $this->db->escape($code) . "'");
	}
