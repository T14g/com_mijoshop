...
class ModelCommonEdit extends Model {
    ...
    public function changeStatus($type, $ids, $status, $extension = false) {
        ...
        if($extension){
            foreach($ids as $id) {
                $db->run("UPDATE #__mijoshop_setting SET `value` = {$status} WHERE `code` = '{$id}' AND `key` = '{$id}_status'", 'query');
            }
        }
        else{
            foreach($ids as $id) {
                $db->run("UPDATE #__mijoshop_{$type} SET status = {$status} WHERE {$type}_id = {$id}", 'query');
            }
        }
    }
    ...
    public function updateOcLang($old_id, $new_id){
        $db = MijoShop::get('db')->run("UPDATE #__mijoshop_language SET language_id = {$new_id} WHERE language_id = {$old_id}", 'query');
    }
