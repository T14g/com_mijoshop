...
jQuery(document).ready(function() {
	...
	jQuery(document).delegate('button[data-toggle=\'image\']', 'click', function() {
		...
		jQuery.ajax({
			url: 'index.php?option=com_mijoshop&route=common/filemanager&view=admin&token=' + getURLVar('token'),
			dataType: 'html',
			beforeSend: function() {
	...
	jQuery(document).delegate('a[data-toggle=\'image\']', 'click', function(e) {
		...
		jQuery('#button-image').on('click', function() {
			...
			jQuery.ajax({
				url: 'index.php?option=com_mijoshop&route=common/filemanager&view=admin&token=' + getURLVar('token') + '&target=' + jQuery(element).parent().find('input').attr('id') + '&thumb=' + jQuery(element).attr('id'),
				dataType: 'html',
				beforeSend: function() {
					jQuery('#button-image i').replaceWith('<i class="fa fa-circle-o-notch fa-spin"></i>');
					jQuery('#button-image').prop('disabled', true);
