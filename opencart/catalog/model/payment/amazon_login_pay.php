...
class ModelPaymentAmazonLoginPay extends Model
{
    ...
    public function addCustomer($data)
    {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int) $customer_group_id . "', store_id = '" . (int) $this->config->get('config_store_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = $this->token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int) $data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '1', approved = '" . (int) !$customer_group_info['approval'] . "', date_added = NOW()");
    ...
    public function addAmazonOrder($order_id, $amazon_authorization_id, $capture_status, $total, $currency_code)
    {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "amazon_login_pay_order` SET `order_id` = '" . $order_id . "', `amazon_order_reference_id` = '" . $this->db->escape($this->session->data['lpa']['AmazonOrderReferenceId']) . "', `amazon_authorization_id` = '" . $this->db->escape($amazon_authorization_id) . "', `modified` = now(), `capture_status` = '" . $this->db->escape($capture_status) . "', `currency_code` = '" . $this->db->escape($currency_code) . "', `total` = '" . $total . "'");
    ...
    protected function getTransactions($amazon_login_pay_order_id, $currency_code)
    {
    ...
    protected function validateResponse($action, $details)
    {
    ...
    protected function calculateStringToSignV2(array $parameters, $url)
    {
        $data = 'POST';
        $data .= "\n";
    ...
    protected function buildMessage($json)
    {
    ...
    protected function parseIpnMessage($ipnMsg)
    {
    ...
    protected function getXmlFromIpnMessage($ipnMsg)
    {
    ...
    protected function getField($fieldName, $snsMessage)
    {
